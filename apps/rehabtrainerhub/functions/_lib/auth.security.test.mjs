...
const popup = await AuthPopupHtml('https://trainerhub.cc/', token, ToPublicUser(user)).text();
assert.equal(popup.includes('auth_token'), false);
assert.equal(popup.includes('https://trainerhub.cc/'), true);
const popupResponse = AuthPopupHtml('https://trainerhub.cc/', token, ToPublicUser(user));
