...
export function MotorTrainingRulesPanel({
  gameId,
  title,
  summaryTitle,
  summaryItems,
  ...
  return (
    <TrainingRulesPanel
      ...
      sections={GetMotorRuleSections(gameId, lang)}
  ...
  switch (gameId) {
    ...
    case 'memory-match':
      return BuildReferenceRules(
        isZh,
        '翻開卡片並找出相同圖案配對，任務包含記憶與視覺搜尋要求。',
        'Flip cards to find matching pairs using memory and visual scanning.',
        [
          '每次可翻開兩張卡片。',
          '若圖案相同會保留配對；不同則會翻回去。',
          '完成所有配對後進入結算。',
    ...
    case 'reaction-time':
      return BuildReferenceRules(
        isZh,
        '等待訊號出現後再點擊，系統會記錄反應時間與提前點擊。',
        'Wait for the signal, then click quickly; the system records response time and early clicks.',
        [
          '訊號出現前不要提前點擊。',
          '看到開始訊號後立刻點擊目標區。',
          '完成設定題數後進入結算。',
    ...
    default:
      return BuildReferenceRules(
        isZh,
        '依畫面提示完成注意、控制與反應任務。',
        'Follow the on-screen attention, control, and response task.',
        ['完成設定目標後進入結算。'],
