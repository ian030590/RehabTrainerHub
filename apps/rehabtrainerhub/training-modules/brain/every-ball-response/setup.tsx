...
>({
  ...
  loadComponent: async () => {
    const [{ EveryBallResponsePage }, { LanguageProvider }, { HashRouter }] = await Promise.all([
      import('../pages/EveryBallResponsePage'),
      import('../i18n'),
      import('react-router-dom'),
    ]);
    return function EveryBallResponseNativeRuntime(props: EveryBallResponsePageProps) {
      return (
        <HashRouter>
          <LanguageProvider>
            <EveryBallResponsePage {...props} />
          </LanguageProvider>
        </HashRouter>
      );
    };
