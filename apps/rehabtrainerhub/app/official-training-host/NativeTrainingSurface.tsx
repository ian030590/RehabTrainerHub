...
>(function NativeTrainingSurface({
  ...
  const ensureEngine = async (): Promise<PreparedTrainingEngine<Record<string, unknown>>> => {
    ...
    if (!enginePromiseRef.current) {
      ...
      const loadPromise = setup.loadEngine({
        ...
        // The rules view remains visible while its engine preloads. Once the
        // engine is ready, leave the transient loading state so the existing
        // Start button becomes available without changing its UI.
        setPhase('rules');
        return engine;
      }).catch((error) => {
    ...
    if (phase === 'config' || phase === 'idle') {
      ...
      return (
        <>
          ...
          <button data-training-action="rules" type="button" onClick={() => {
            setPhase('rules');
  ...
  return (
    <main className="native-training-surface" data-module-id={manifest.id} data-phase={phase}>
