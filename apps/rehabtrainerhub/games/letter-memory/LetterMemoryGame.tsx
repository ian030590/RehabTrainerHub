...
export function LetterMemoryGame({ onExit }: LetterMemoryGameProps) {
  ...
  const [phase, setPhase] = useState<LetterPhase>('menu');
  const [recallLength, setRecallLength] = useState<number>(4);
  const [rounds, setRounds] = useState(6);
  const [presentationSpeedMs, setPresentationSpeedMs] = useState(1500);
  ...
  const completeSession = useCallback((finalRecords: LetterTrialRecord[]) => {
    ...
    void SaveTrainingSessionRecord({
      ...
      gameTitle: isZh ? '字母記憶更新 (Letter Memory)' : 'Letter Memory Updating Task',
      category: 'memory',
  ...
  const handleLetterTap = useCallback((letter: string) => {
    if (streamState !== 'recall' || !activeSequence) return;
    if (userSelection.length >= activeSequence.recallLength) return;
  ...
  const handleBackspace = useCallback(() => {
    if (streamState !== 'recall' || userSelection.length === 0) return;
  ...
  const handleSubmitRecall = useCallback(() => {
    ...
    if (soundEnabled) {
      if (isFullyCorrect) PlaySuccessSound(jsPsychRef);
  ...
  return (
    ...
    >
      ...
      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '字母記憶更新 (Letter Memory)' : 'Letter Memory Task'}
          subtitle={isZh ? '居家工作記憶流持續動態更新練習' : 'Running working memory updating activity'}
        >
          <TrainingConfigSection title={isZh ? '追蹤字母數' : 'Tracked Letters'}>
            <TrainingConfigOptionGroup
              options={configOptions.recOptions}
              value={String(recallLength)}
              onChange={(val) => setRecallLength(Number(val))}
            />
          </TrainingConfigSection>
          ...
          <TrainingConfigSection title={isZh ? '題目數量' : 'Trial Count'}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '8px 0' }}>
              ...
              <span style={{ minWidth: '60px', fontWeight: 700, fontSize: '18px' }}>
                {rounds} {isZh ? '題' : 'sequences'}
              </span>
            </div>
          </TrainingConfigSection>

          <TrainingConfigSection title={isZh ? '字母出現間隔' : 'Presentation Pace'}>
            <TrainingConfigOptionGroup
              options={configOptions.spdOptions}
              value={String(presentationSpeedMs)}
              onChange={(val) => setPresentationSpeedMs(Number(val))}
            />
          </TrainingConfigSection>
      ...
      {phase === 'playing' && (
        ...
        >
          ...
          >
            ...
            {(streamState === 'recall' || streamState === 'feedback') && activeSequence && (
          ...
          {streamState === 'recall' ? (
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginTop: '4px' }}>
                ...
                <button
                  ...
                  disabled={userSelection.length < recallLength}
                  style={{
                    height: '60px',
                    fontSize: '22px',
                    fontWeight: 900,
            ...
            <div style={{ height: '124px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
