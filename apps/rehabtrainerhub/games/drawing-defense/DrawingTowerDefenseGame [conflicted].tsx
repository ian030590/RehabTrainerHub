...
export function DrawingTowerDefenseGame({ onExit }: DrawingTowerDefenseGameProps) {
    ...
    const backgroundSummary = backgroundMode === 'stars' ? t('drawing.background.stars') : backgroundMode === 'color' ? backgroundColor : t('drawing.background.customImage');
    const backgroundModeLabel = backgroundMode === 'stars' ? t('drawing.background.image') : backgroundMode === 'color' ? t('drawing.background.color') : t('drawing.background.customImage');
    const backgroundStyle = useMemo<CSSProperties>(() => {
        if (backgroundMode === 'stars')
            return { backgroundImage: starSkyBackgroundImage };
    ...
    const recordEnemyOutcome = useCallback((enemy: Enemy, defeatedEnemy: boolean) => {
        ...
        if (!result || result.Reaction_Time_Seconds !== null)
            return;
    ...
    const startGame = useCallback(async () => {
        ...
        if (!app)
            return;
        await jsPsychLifecycleRef.current?.start({ moduleId: 'motor:drawing-defense', onStart: () => {
        ...
        const init = async () => {
            ...
            app.ticker.add((ticker: Ticker) => {
                ...
                if (isTimeUnlimited || metrics.elapsed < targetGameDurationSec) {
                    const shouldSpawn = metrics.spawned === 0 ||
    ...
    useEffect(() => {
        ...
        const onPointerMove = (event: PointerEvent) => {
            if (!isDrawingRef.current || phaseRef.current !== 'playing')
                return;
            event.preventDefault();
...
function RecognizeShape(strokes: Point[][], strictness: number): ShapeId | null {
    ...
    if (rawPoints.length < 6)
        return null;
    if (LooksLikeCircle(rawPoints, strictness))
        return 'circle';
    const box = GetBox(rawPoints);
    const tolerance = ShapeTolerance(strictness);
    if (LooksLikeIntersectingCross(usableStrokes, box, tolerance))
        return 'cross';
    const polygonRecognition = RecognizePolygonByRdpCorners(usableStrokes, rawPoints, box, strictness);
    if (polygonRecognition)
        return polygonRecognition;
    if (LooksLikeCross(usableStrokes, rawPoints, box, tolerance))
        return 'cross';
    ...
    for (const template of gestureTemplates) {
        if (template.shape === 'square' || template.shape === 'triangle')
            continue;
    ...
    const adjustedThreshold = best?.shape === 'cross'
        ? threshold - 0.06
        : threshold;
...
function LooksLikeCircle(points: Point[], strictness: number): boolean {
    ...
    if (maxSize < 24 || minSize / Math.max(1, maxSize) < 0.45)
        return false;
    ...
    return (closedness <= closureLimit &&
        areaRatio >= 0.48 &&
        areaRatio <= 0.95 &&
...
function RecognizePolygonByRdpCorners(strokes: Point[][], points: Point[], box: ReturnType<typeof GetBox>, strictness: number): ShapeId | null {
...
function GetRdpPolygonCorners(strokes: Point[][], points: Point[], box: ReturnType<typeof GetBox>, strictness: number): Point[] {
    if (points.length < 6)
        return [];
    const width = box.maxX - box.minX;
    const height = box.maxY - box.minY;
    ...
    if (maxSize < 24 || minSize / Math.max(1, maxSize) < 0.32)
        return [];
...
function MergeClosedEndpoint(points: Point[], closeDistance: number): Point[] {
    ...
    if (Distance(first, last) > closeDistance)
        return points;
    return [
...
function LooksLikeIntersectingCross(strokes: Point[][], box: ReturnType<typeof GetBox>, tolerance: number): boolean {
    ...
    for (let i = 0; i < diagonalLines.length - 1; i += 1) {
        for (let j = i + 1; j < diagonalLines.length; j += 1) {
            ...
            if (diff < 48 - tolerance * 12 || diff > 132 + tolerance * 12)
                continue;
            const intersection = LineIntersection(first.first, first.last, second.first, second.last);
            if (!intersection)
                continue;
            if (Distance(intersection, center) > centerLimit)
                continue;
            if (!PointInsideSegment(intersection, first.first, first.last, maxSize * 0.04))
                continue;
            if (!PointInsideSegment(intersection, second.first, second.last, maxSize * 0.04))
                continue;
            if (!PointIsInternalToSegment(intersection, first.first, first.last, minInternalRatio))
                continue;
            if (!PointIsInternalToSegment(intersection, second.first, second.last, minInternalRatio))
                continue;
            return true;
        }
    }
...
function LineIntersection(aStart: Point, aEnd: Point, bStart: Point, bEnd: Point): Point | null {
    const denominator = (aStart.x - aEnd.x) * (bStart.y - bEnd.y) -
