...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "在停止訊號出現時煞住原本的反應。",
    "en": "Brake the planned response when the stop signal appears."
  },
  "stimulus": {
    "zh": "先出現一般反應刺激，部分回合接著出現停止訊號。",
    "en": "A go stimulus appears first; some trials then show a stop signal."
  },
  "response": {
    "zh": "沒有停止訊號就反應，看到停止訊號就抑制。",
    "en": "Respond without a stop signal and withhold when it appears."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:stop-signal';
  ...
  useEffect(() => {
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-stop-signal',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="stop-signal" inert={!complete || undefined} aria-label={"煞車抑制反應 / Stop-Signal"}>
      {children}
    </div>
  );
}
