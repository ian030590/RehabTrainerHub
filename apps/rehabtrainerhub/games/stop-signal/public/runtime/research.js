(function () {
  ...
  function task(row) { return ['NoSS_practice','practice','test'].indexOf(row.exp_stage) >= 0 && row.trial_id === 'stim'; }
  function base(row) {
    ...
    return { trial_id: 'response', exp_stage: 'test', trial: row.trial_index, practice: row.exp_stage !== 'test', correct: correct, rt: typeof row.rt === 'number' && Number.isFinite(row.rt) && row.rt >= 0 ? row.rt : null, key: typeof row.key_press === 'number' ? row.key_press : null, setting: configuredValue };
  }
  function responses(row) {
    try { return JSON.parse(row.responses || '{}'); } catch { return {}; }
  }
  window.rehabResearchRows = function(rows) {
    return rows.filter(task).map(function(r) { var stop = r.SS_trial_type === 'stop'; return Object.assign(base(r), { correct: stop ? r.key_press === -1 : r.key_press === r.correct_response, stop: stop, stopDelayMs: stop ? r.SS_delay : null, condition: r.exp_stage === 'test' ? r.condition === 'high' ? 1 : 0 : null, omission: !stop && r.key_press === -1 }); });
  };
