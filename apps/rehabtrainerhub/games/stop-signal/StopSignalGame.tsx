...
export function StopSignalGame({ onExit }: StopSignalGameProps) {
    ...
    const [phase, setPhase] = useState<StopSignalPhase>('rules');
    useTrainingAbort({ active: phase === 'playing', onAbort: handleExitTraining });
    const [difficulty, setDifficulty] = useState<StopSignalDifficulty>(GetHostedGameSetting<StopSignalDifficulty>('difficulty'));
    ...
    const completeSession = useCallback((finalTrials: StopSignalTrial[]) => {
        ...
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'stop-signal', gameId: 'stop-signal', gameTitle: isZh ? '煞車抑制反應 (Stop-Signal)' : 'Stop-Signal Response Inhibition', difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { category: 'attention',
...
notes: isZh
    ...
    const startGame = useCallback(() => {
        ...
        setIsStopCued(false);
        setPhase('playing');
        jsPsychLifecycleRef.current?.start({ moduleId: "stop-signal", onStart: () => undefined });
    ...
    return (<div ref={fullscreenRootRef} className="cognitive-reference-game" style={{
      ...
      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="stop-signal" onStart={() => void startGame()} onBack={() => RequestHubTrainingConfiguration()} title={document.title}/>)}
      ...
      {phase === 'playing' && (<div style={{
          ...
          <div style={{
            ...
            {isFixation ? (<span style={{ fontSize: '64px', fontWeight: 900, color: 'var(--text-secondary)' }}>+</span>) : activeTrial ? (<>
                {/* Direction Arrow */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {activeTrial.direction === 'left' ? (<svg viewBox="0 0 100 100" width="140" height="140">
                      <polygon points="65,18 25,50 65,82 65,62 85,62 85,38 65,38" fill={isStopCued ? '#94a3b8' : 'var(--accent)'} stroke={isStopCued ? '#64748b' : 'var(--accent-hover)'} strokeWidth="3"/>
                    </svg>) : (<svg viewBox="0 0 100 100" width="140" height="140">
                      <polygon points="35,18 75,50 35,82 35,62 15,62 15,38 35,38" fill={isStopCued ? '#94a3b8' : 'var(--accent)'} stroke={isStopCued ? '#64748b' : 'var(--accent-hover)'} strokeWidth="3"/>
                    </svg>)}
                </div>

                {/* Stop Signal Overlay */}
                {isStopCued && (<div style={{
                        position: 'absolute',
                        display: 'flex',
                        flexDirection: 'column',
                        alignItems: 'center',
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <button type="button" onClick={() => handleUserDirection('left')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                minHeight: '76px',
                fontSize: '24px',
                fontWeight: 900,
                borderRadius: '18px',
              ...
              <button type="button" onClick={() => handleUserDirection('right')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                minHeight: '76px',
                fontSize: '24px',
                fontWeight: 900,
                borderRadius: '18px',
