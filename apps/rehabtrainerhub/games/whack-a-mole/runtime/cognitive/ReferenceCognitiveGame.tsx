...
import { BuildWhackResultStats,CreateWhackState,DrawWhack,ExpireWhackTarget,HandleWhackTap,IsWhackAutoSuccess,ShowWhackTarget,UpdateWhackTimedState,} from '../../TargetClickGame';
...
export function ReferenceCognitiveGame({ gameId, onExit, trainingModuleId = 'thinking-training', trainingConfigLabel, }: ReferenceCognitiveGameProps) {
    ...
    const finishGame = useCallback((gameResult: GameResult) => {
        ...
        setResult(record);
        setPhase('results');
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'whack-a-mole', gameId: 'whack-a-mole', gameTitle: metaTitle, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { userName: participantId,
...
detailRows: timingData.detailRows } });
    ...
    const startGame = useCallback(async () => {
        ...
        await jsPsychLifecycleRef.current?.start({ moduleId: `brain:${gameId}`, onStart: () => {
                ...
                if (stateRef.current?.kind === 'whack-a-mole')
                    ScheduleNextWhackTarget(stateRef.current);
            } });
    ...
    function HandleWhackCellTap(index: number, tapMs: number) {
        if (phaseRef.current !== 'playing')
            return;
        const state = stateRef.current;
        if (!state || state.kind !== 'whack-a-mole')
            return;
        ...
        if (tapResult.targetCompleted)
            ScheduleNextWhackTarget(state);
        PlayFeedbackForCountChange(feedbackBefore, GetFeedbackCounts(state), jsPsychRef);
        renderRef.current();
    }
    function ScheduleNextWhackTarget(state: Extract<CognitiveGameState, {
        kind: 'whack-a-mole';
    }>) {
        const delayMs = Math.max(0, state.nextTargetAt - performance.now());
        jsPsychRef.current?.pluginAPI.setTimeout(() => {
            if (phaseRef.current !== 'playing' || stateRef.current !== state || state.activeIndex !== null)
                return;
            if (!ShowWhackTarget(state, performance.now()))
                return;
            renderRef.current();
            FlushPixiRender();
            ...
            jsPsychRef.current?.pluginAPI.setTimeout(() => {
                if (phaseRef.current !== 'playing' || stateRef.current !== state || state.targetStartedAt !== onsetMs)
                    return;
                const feedbackBefore = GetFeedbackCounts(state);
                ...
                app.ticker.add((ticker: Ticker) => {
                    ...
                    if (limit !== null && metricsRef.current.elapsed >= limit) {
                        if (stateRef.current?.kind === 'whack-a-mole' && stateRef.current.activeIndex !== null) {
                            ...
                            if (finalTrial)
                                FinishCognitiveTrial('target', finalTrial);
                        }
...
function CreateInitialState(gameId: ReferenceGameId, difficulty: Difficulty, reactionTrials: number, simonLives: number): CognitiveGameState { return CreateWhackState(difficulty); }
function UpdateTimedState(state: CognitiveGameState | null, elapsed: number, render: () => void, finishGame: (result: GameResult) => void, onSimonInputStart: () => void) {
    ...
    if (state.kind === 'whack-a-mole')
        UpdateWhackTimedState(state, elapsed, render);
    ;
}
function IsAutoSuccess(state: CognitiveGameState | null) { if (!state)
    return false; return IsWhackAutoSuccess(state); }
function GetFeedbackCounts(state: CognitiveGameState): {
    success: number;
    errors: number;
} { return { success: state.hits, errors: state.misses }; }
