...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "在不同距離或位置快速辨讀字母。",
    "en": "Read letters quickly across changing distances or positions."
  },
  "stimulus": {
    "zh": "字母會分布在中央與周邊位置，大小或距離可能改變。",
    "en": "Letters appear centrally and peripherally with changing size or distance."
  },
  "response": {
    "zh": "辨認並輸入看到的字母。",
    "en": "Identify and enter the letter you see."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:hart-chart';
  ...
  useEffect(() => {
    ...
    const target = () => rootRef.current?.querySelector<HTMLElement>("input,canvas") ?? rootRef.current;
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-hart-chart',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="hart-chart" inert={!complete || undefined} aria-label={"Hart 字母圖 / Hart Chart"}>
      {children}
    </div>
  );
}
