...
function GetRuleSections(gameId: string, lang: 'zh' | 'en') {
    ...
    switch (gameId) {
        case 'keep-track':
            return BuildRules(isZh, '在快速流動的多類別單詞中，持續更新並維持各指定目標類別「最後出現」的單詞。', 'Continuously track and update the last presented item for each designated target category across a rapid word stream.', ['測驗開始前會公佈本回合的追蹤類別（例如：動物、水果）。', '播放單詞流時，只需專注記住目標類別中最新出現的詞彙。', '播畢後從大尺寸卡片中點選各目標類別最後出現的單詞。'], ['Target categories are revealed before the stream begins (e.g. Animals, Fruits).', 'Focus mental attention on updating only the newest word seen for each target category.', 'Select the final word for each target category from clear large choice cards.']);
        default:
