...
export function KeepTrackGame({ onExit }: KeepTrackGameProps) {
  ...
  const [categoryCount, setCategoryCount] = useState<number>(() => (
    Number(hostedSettings?.categoryCount ?? defaultCategoryCount)
  ));
  const [rounds, setRounds] = useState<number>(() => (
    Number(hostedSettings?.rounds ?? defaultRounds)
  ));
  const [wordDurationMs, setWordDurationMs] = useState<number>(() => (
    Number(hostedSettings?.wordDurationMs ?? defaultWordDurationMs)
  ));
  ...
  const [phase, setPhase] = useState<KeepTrackPhase>('menu');
  ...
  const [roundScores, setRoundScores] = useState<Array<{ correct: number; total: number; rt: number }>>([]);
  ...
  useEffect(() => {
    if (phase !== 'playing' || presentationState !== 'streaming' || !currentRound) return;
  ...
  return (
    ...
    >
      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '類別記憶追蹤任務' : 'Keep Track Task'}
          subtitle={
            isZh
              ? '即時更新與維持多目標類別的最後單詞，鍛鍊工作記憶動態更新能力'
              : 'Continuously update and maintain the last words of designated categories'
          }
        >
          <TrainingConfigSection title={isZh ? '追蹤類別數量' : 'Target Category Count'}>
            <TrainingConfigOptionGroup
              name="categoryCount"
              options={[
                { value: '2', label: isZh ? '2 種目標類別（入門）' : '2 Categories (Easy)' },
                { value: '3', label: isZh ? '3 種目標類別（標準經典）' : '3 Categories (Standard)' },
                { value: '4', label: isZh ? '4 種目標類別（挑戰進階）' : '4 Categories (Hard)' },
              ]}
              selectedValue={String(categoryCount)}
              onChange={(val) => setCategoryCount(Number(val))}
            />
          </TrainingConfigSection>

          <TrainingConfigSection title={isZh ? '單詞呈現時間' : 'Word Presentation Time'}>
            <TrainingConfigOptionGroup
              name="wordDurationMs"
              options={[
                { value: '1400', label: isZh ? '1.4 秒（緊湊）' : '1.4 s (Fast)' },
                { value: '1800', label: isZh ? '1.8 秒（標準）' : '1.8 s (Standard)' },
                { value: '2400', label: isZh ? '2.4 秒（從容）' : '2.4 s (Relaxed)' },
              ]}
              selectedValue={String(wordDurationMs)}
              onChange={(val) => setWordDurationMs(Number(val))}
            />
          </TrainingConfigSection>
      ...
      {phase === 'playing' && currentRound && (
        ...
        >
          ...
          {presentationState === 'target-preview' && (
            <div
              style={{
                width: '100%',
            ...
            >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'center', marginBottom: '32px' }}>
                {currentRound.targetCategories.map((cat) => (
                  <div
                    key={cat.id}
                    style={{
                      display: 'flex',
          ...
          {(presentationState === 'streaming' || presentationState === 'blank') && (
            <div
              style={{
                display: 'flex',
                flexDirection: 'column',
            ...
            >
              ...
              >
                ...
                {presentationState === 'blank' && (
                  <span style={{ fontSize: '36px', color: 'var(--text-secondary)' }}>+</span>
                )}
              </div>
                    ...
                    >
                      {optionChoices.map((choice) => (
                        <button
                          key={choice.id}
                          type="button"
                          onClick={() => handleAnswerChoice(choice)}
                          style={{
                            minHeight: '84px',
                            background: 'var(--bg)',
