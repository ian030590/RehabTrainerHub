(function () {
  ...
  window.rehabConfigure = function(settings, timeline) {
    ...
    if (value === undefined) value = 800;
    if (!Number.isFinite(value) || value < 200 || value > 1600 || Math.abs((value - 200) / 100 - Math.round((value - 200) / 100)) > 1e-8) throw new Error('Invalid itemExposureMs');
  ...
  window.rehabResearchRows = function(rows) {
    return rows.filter(function(r) { return r.exp_stage === 'test' && r.trial_id === 'response'; }).map(function(r) { return Object.assign(base(r), { load: r.num_digits, reverse: r.condition === 'reverse' }); });
  };
})();
