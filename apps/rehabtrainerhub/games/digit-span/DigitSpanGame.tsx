...
export function DigitSpanGame({ onExit }: DigitSpanGameProps) {
  ...
  const handleKeypadPress = useCallback((digit: number) => {
    ...
    if (userInput.length >= activeSequenceRef.current.length) return;
  ...
  useEffect(() => {
    if (phase !== 'playing' || subPhase !== 'input') return;
  ...
  return (
    ...
    >
      ...
      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '數字廣度記憶（Digit Span）' : 'Digit Span Memory Capacity'}
          subtitle={isZh ? '記住依序出現的數字串，並依規定順序（順向或逆向）在數字鍵盤輸入。' : 'Memorize digits in sequence and recall them forward or backward.'}
        >
          <TrainingConfigSection
            title={isZh ? '記憶模式' : 'Span Mode'}
            description={isZh ? '順向為依原順序回憶；逆向為依相反倒序回憶。' : 'Forward recalls original order; backward recalls reverse order.'}
          >
            <TrainingConfigOptionGroup
              options={[
          ...
          <TrainingConfigSection
            title={isZh ? '起始位數' : 'Starting Length'}
            description={isZh ? '練習開始時的數字串長度。' : 'Starting digit sequence length.'}
          >
            <TrainingConfigOptionGroup
              options={[
                { value: 2, label: isZh ? '2 位數（入門）' : '2 digits' },
                { value: 3, label: isZh ? '3 位數（標準）' : '3 digits' },
                { value: 4, label: isZh ? '4 位數（進階）' : '4 digits' },
              ]}
              value={startLength}
              onChange={(val) => {
          ...
          >
            <TrainingConfigOptionGroup
              options={[
                ...
                { value: 15, label: isZh ? '15 題（進階）' : '15 trials' },
              ]}
              value={rounds}
          ...
          <TrainingConfigSection
            title={isZh ? '數字切換速度' : 'Presentation Speed'}
            description={isZh ? '每個數字在畫面呈現與停留的速度。' : 'Display speed per digit.'}
          >
            <TrainingConfigOptionGroup
              options={[
              ...
              value={digitSpeedMs}
              onChange={(val) => setDigitSpeedMs(Number(val))}
            />
          </TrainingConfigSection>

          <TrainingConfigSection
            title={isZh ? '聲音回饋' : 'Audio Feedback'}
            description={isZh ? '在數字切換與作答時是否播放提示音效。' : 'Play sound cues for numbers and answers.'}
          >
            <TrainingConfigOptionGroup
              options={[
      ...
      {phase === 'rules' && (
        <BrainTrainingRulesPanel
          gameId="digit-span"
          title={isZh ? '數字廣度記憶（Digit Span）說明' : 'Digit Span Rules'}
      ...
      {phase === 'playing' && (
        <div style={{
          ...
          <div style={{
            ...
            {(subPhase === 'input' || subPhase === 'feedback') && (
              spanMode === 'forward'
          ...
          <div style={{
            ...
            <button
              type="button"
              disabled={subPhase !== 'input' || userInput.length === 0}
