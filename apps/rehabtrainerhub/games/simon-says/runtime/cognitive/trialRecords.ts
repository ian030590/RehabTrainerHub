...
export function CreateSimonTrialRecord(trialNumber: number, memoryLength: number, correct: boolean, attemptStartedAtSeconds: number | null, endedAtSeconds: number): SimonTrialRecord {
    return {
        ...
        memoryLength,
        correct,
        durationMs: GetElapsedMilliseconds(attemptStartedAtSeconds === null ? null : attemptStartedAtSeconds * 1000, endedAtSeconds * 1000),
    };
}
...
export function CreateSimonState(difficulty: Difficulty, configuredLives: number, random: () => number = Math.random): SimonState {
...
export function HandleSimonTap(state: SimonState, index: number, elapsed: number, random: () => number = Math.random): SimonTapResult {
    ...
    if (state.status !== 'input' || index < 0 || index > 3)
        return ignored;
    state.pressedIndex = index;
    state.pressedStartedAt = elapsed;
    if (state.sequence[state.inputIndex] !== index) {
        const trial = CreateSimonTrialRecord(state.trials.length + 1, state.sequence.length, false, state.attemptStartedAt, elapsed);
    ...
    const trial = CreateSimonTrialRecord(state.trials.length + 1, state.sequence.length, true, state.attemptStartedAt, elapsed);
    state.trials.push(trial);
    state.attemptStartedAt = null;
