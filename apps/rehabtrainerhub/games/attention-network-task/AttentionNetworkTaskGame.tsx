...
export function AttentionNetworkTaskGame({ onExit }: AttentionNetworkTaskGameProps) {
    ...
    const [phase, setPhase] = useState<AntPhase>('rules');
    useTrainingAbort({ active: phase === 'playing', onAbort: handleExitTraining });
    const [difficulty, setDifficulty] = useState<'standard' | 'spatial' | 'double'>(GetHostedGameSetting<'standard' | 'spatial' | 'double'>('difficulty'));
    ...
    const completeSession = useCallback((finalTrials: AntTrial[]) => {
        ...
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'attention-network-task', gameId: 'attention-network-task', gameTitle: isZh ? '注意網絡測驗 (ANT)' : 'Attention Network Task', difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { category: 'attention',
...
notes: isZh
    ...
    const handleResponse = useCallback((chosenDirection: ArrowDirection) => {
        if (phase !== 'playing' || trialStage !== 'target' || !activeTrial || currentTrialAnsweredRef.current)
            return;
    ...
    const startGame = useCallback(() => {
        ...
        setTrialFeedback(null);
        setPhase('playing');
        jsPsychLifecycleRef.current?.start({ moduleId: "attention-network-task", onStart: () => undefined });
    ...
    return (<div ref={fullscreenRootRef} className="cognitive-reference-game" style={{
      ...
      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="attention-network-task" onStart={() => void startGame()} onBack={() => RequestHubTrainingConfiguration()} title={document.title}/>)}
      ...
      {phase === 'playing' && (<div style={{
          ...
          <div style={{
            ...
            <div style={{ height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {trialStage === 'cue' && activeTrial && ((activeTrial.cueType === 'double-cue') ||
                (activeTrial.cueType === 'spatial-cue' && activeTrial.targetLocation === 'top')) && <AntAsterisk />}

              {trialStage === 'target' && activeTrial && activeTrial.targetLocation === 'top' && (<AntStimulusRow direction={activeTrial.targetDirection} flanker={activeTrial.flankerType}/>)}
            </div>

            {/* Center Fixation Slot */}
            <div style={{ height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {trialStage === 'cue' && activeTrial?.cueType === 'center-cue' ? (<AntAsterisk />) : (<span style={{ fontSize: '48px', fontWeight: 900, color: 'var(--text-secondary)' }}>+</span>)}
            </div>

            {/* Bottom Location Slot */}
            <div style={{ height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {trialStage === 'cue' && activeTrial && ((activeTrial.cueType === 'double-cue') ||
                (activeTrial.cueType === 'spatial-cue' && activeTrial.targetLocation === 'bottom')) && <AntAsterisk />}

              {trialStage === 'target' && activeTrial && activeTrial.targetLocation === 'bottom' && (<AntStimulusRow direction={activeTrial.targetDirection} flanker={activeTrial.flankerType}/>)}
            </div>
          </div>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <button type="button" onClick={() => handleResponse('left')} disabled={trialStage !== 'target' || currentTrialAnsweredRef.current} style={{
                minHeight: '76px',
                fontSize: '24px',
                fontWeight: 900,
                borderRadius: '18px',
              ...
              <button type="button" onClick={() => handleResponse('right')} disabled={trialStage !== 'target' || currentTrialAnsweredRef.current} style={{
                minHeight: '76px',
                fontSize: '24px',
                fontWeight: 900,
                borderRadius: '18px',
...
function AntAsterisk() {
    return (<span style={{ fontSize: '42px', fontWeight: 900, color: '#fbbf24', lineHeight: 1 }}>*</span>);
}
function AntArrow({ direction }: {
    direction: ArrowDirection;
}) {
    return (<span style={{ fontSize: '38px', fontWeight: 900, color: 'var(--accent)', margin: '0 4px', lineHeight: 1 }}>
      {direction === 'left' ? '←' : '→'}
    </span>);
}
function AntDash() {
    return (<span style={{ fontSize: '38px', fontWeight: 900, color: 'var(--text-secondary)', margin: '0 6px', lineHeight: 1 }}>
      —
    </span>);
}
function AntStimulusRow({ direction, flanker, }: {
    direction: ArrowDirection;
...
function GenerateAntTrials(rounds: number, difficulty: 'standard' | 'spatial' | 'double'): AntTrial[] {
    ...
    const cuePool: AntCueCondition[] = difficulty === 'spatial'
