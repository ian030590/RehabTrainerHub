...
function GetRuleSections(gameId: string, lang: 'zh' | 'en') {
    ...
    switch (gameId) {
        case 'attention-network-task':
            return BuildRules(isZh, '注視中央目標箭頭，判斷其指向左方或右方並點擊大箭頭按鈕。', 'Focus on the central target arrow, determine whether it points left or right, and tap the large arrow buttons.', ['中央箭頭兩側可能伴隨同向（一致）或反向（衝突）的干擾箭頭。', '出現星號提示時，注意提示出現的位置，可加速捕捉箭頭。', '請在兼顧正確率的前提下，以最快速度點選對應方向。'], ['Flanking arrows may point in the same (congruent) or opposite (incongruent) direction.', 'Asterisk cues indicate timing or spatial location to help prepare attention.', 'Respond as quickly and accurately as possible using the large on-screen buttons.']);
        default:
