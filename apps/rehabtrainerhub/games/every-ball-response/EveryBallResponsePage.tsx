...
export function EveryBallResponsePage({ onExit }: {
    onExit?: () => void;
} = {}) {
    const { lang } = useT();
    ...
    const [phase, setPhaseState] = useState<Phase>('rules');
    ...
    const canRetryMediaPermission = CanRetryMediaPermission(mediaPermission.status);
    ...
    useEffect(() => {
        ...
        if (typeof hostedSettings.rounds === 'number')
            setTrialCount(hostedSettings.rounds);
        setPhase('rules');
    ...
    const drawVisual = useCallback((nextVisual: VisualState) => {
        ...
        if (app)
            DrawEveryBallScene(app, nextVisual, labels);
    }, [labels]);
    useEffect(() => {
    ...
    const completeSession = useCallback((data: EveryBallExperimentData, activeLabels: EveryBallLabels, level: LevelDefinition, modeLabel: string) => {
        void stopInput();
        const now = new Date();
    ...
    return (<div ref={fullscreenRootRef} className={`every-ball every-ball-phase-${phase}`}>
      ...
      {phase === 'rules' && (<div className="training-panel every-ball-panel">
          <TrainingRulesPanel label={labels.rulesLabel} title={selectedLevelLabels.title} summaryTitle={labels.title} summaryItems={summaryItems} sections={[
                {
            ...
            ]} startLabel={labels.start} backLabel={labels.backSettings} onStart={() => void startSession()} onBack={showConfiguration}/>
        </div>)}

      {phase === 'initializing' && (<div className="every-ball-status-panel" role="status">
          <strong>{statusMessage}</strong>
          <span>{labels.permissionPrivacy}</span>
        </div>)}

      {phase === 'playing' && (<div className="every-ball-hud">
          ...
          {inputMode === 'microphone' && (<div className="every-ball-meter" aria-label={labels.microphoneLevel}>
      ...
      {phase === 'playing' && (<MobileActionControls className="every-ball-mobile-actions" actions={[
                {
                    id: 'clap',
      ...
      {phase === 'results' && summary && (<div className="experiment-container experiment-container-scrollable every-ball-results-container">
          <div className="experiment-results every-ball-results">
            ...
            <table className="results-table every-ball-results-table">
              ...
              <tbody>
                {summary.trials.map((trial) => (<tr key={trial.Trial_Number}>
            ...
            <TrainingResultActions backLabel={labels.returnHome} onBackHome={() => void exitToAttentionTraining()} hubLabel={labels.returnLobby}/>
          </div>
        </div>)}
    </div>);
}
...
function CreateBallSequence(levelId: LevelDefinition['id'], trialCount: number): BallId[] {
    if (levelId === 1)
        return Array.from({ length: trialCount }, () => 'basketball');
    const sequence: BallId[] = [];
    const targetCount = Math.max(1, Math.round(trialCount * (levelId === 2 ? 0.56 : 0.62)));
    for (let index = 0; index < targetCount; index += 1) {
        if (levelId === 3 && index % 2 === 1)
            sequence.push('soccer');
        else
            sequence.push('basketball');
    }
...
function BuildSessionSummary(trials: TrialRecord[], labels: EveryBallLabels, level: LevelDefinition, inputMode: string, date: Date): SessionSummary {
...
async function CreateCameraRuntime(options: CameraRuntimeOptions, labels: EveryBallLabels): Promise<InputRuntime> {
    ...
    const { handLandmarker, poseLandmarker } = await LoadMediaPipeWithFallback(mediaPipeAssetCandidates, async ({ wasmUrl, handLandmarkerModelUrl, poseLandmarkerLiteModelUrl, }) => {
    ...
    const processFrame = (now: number) => {
        ...
        if (stopped || now - lastDetectionAt < cameraDetectionIntervalMs)
            return;
...
async function CreateMicrophoneRuntime(options: MicrophoneRuntimeOptions, labels: EveryBallLabels): Promise<InputRuntime> {
    ...
    const tick = (now: number) => {
        ...
        if (!belowThreshold || rms < threshold || now - lastTriggerAt < microphoneCooldownMs)
            return;
...
function DrawBall(stage: Container, ball: BallId, x: number, y: number, radius: number, labels: EveryBallLabels): void {
    ...
    if (ball === 'basketball')
        DrawBasketball(container, radius);
    else if (ball === 'soccer')
        DrawSoccerBall(container, radius);
    else if (ball === 'tennis')
        DrawTennisBall(container, radius);
    else
        DrawBeachBall(container, radius);
    const text = new Text({
...
function DrawCameraOverlay(canvas: HTMLCanvasElement, video: HTMLVideoElement, handLandmarks: NormalizedLandmark[][], poseLandmarks?: NormalizedLandmark[]): void {
...
function IsThighContact(handCenters: NormalizedLandmark[], pose?: NormalizedLandmark[]): boolean {
    if (!pose)
        return handCenters.some((hand) => hand.y > 0.62);
    const zones = [
...
function ClassifyTapSound(freqSamples: Uint8Array): ResponseAction {
    ...
    for (let index = 0; index < freqSamples.length; index += 1) {
        if (index < split)
            low += freqSamples[index];
        else
            high += freqSamples[index];
    }
...
function GetExpectedActionLabel(action: ExpectedAction, labels: EveryBallLabels): string {
    if (action === 'clap')
        return labels.actionClap;
    if (action === 'thigh')
        return labels.actionThigh;
...
function RequireElement<T>(element: T | null): T {
    if (!element)
        throw new Error('Required browser element is unavailable.');
    return element;
}
...
function WriteJsPsychData(jsPsych: JsPsych, record: Record<string, unknown>): void {
    try {
        const writer = jsPsych.data as unknown as {
