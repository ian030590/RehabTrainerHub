...
import { Application,Assets,Container,MeshRope,Point,Sprite,Texture,type Ticker,} from 'pixi.js';
import { useCallback,useEffect,useRef,useState } from 'react';
import { Clamp,FormatTestDate } from './gameUtils';
import { MouthTrainingRulesPanel } from './runtime/components/rules/MouthTrainingRulesPanel';
import { JsPsychExternalLifecycle } from './runtime/jsPsychLifecycle';
import { defaultTongueSettings,GetTongueTrainingSettings,SaveTongueTrainingSettings,type TongueTrainingSettings,} from './storage/tongueRehabStorage';
...
const appleTextureUrl = new URL('./textures/apple.svg', import.meta.url).href;
...
const lipLandmarkIndices = Array.from(new Set(FaceLandmarker.FACE_LANDMARKS_LIPS.flatMap((connection) => [connection.start, connection.end])));
export function TongueCatchGame({ onExit }: TongueCatchGameProps) {
    ...
    const calibrationResultsRef = useRef<Array<{
        class_label: TongueClass;
        sample_count: number;
    }>>([]);
    ...
    const [config, setConfig] = useState<TongueTrainingSettings>(() => (activeUser ? GetTongueTrainingSettings(activeUser) : { ...defaultTongueSettings }));
    ...
    useEffect(() => {
        ...
        if (activeUser)
            SaveTongueTrainingSettings(activeUser, config);
    }, [activeUser, config]);
    useEffect(() => {
    ...
    const finishCalibrationStep = useCallback(() => {
        ...
        calibrationResultsRef.current.push({
    ...
    const processFrame = useCallback((now: number) => {
        ...
        if (phaseRef.current !== 'calibration' && phaseRef.current !== 'playing')
            return;
        ...
        if (!video || !landmarker || !featureCanvas || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA)
            return;
    ...
    const startSession = useCallback(async () => {
        ...
        await jsPsychLifecycleRef.current?.start({ moduleId: 'mouth:tongue-catch', onStart: async () => {
                if (appRef.current)
                ...
                try {
                    ...
                    await tf.ready();
                    const landmarker = await LoadMediaPipeWithFallback(mediaPipeAssetCandidates, async ({ wasmUrl, faceLandmarkerModelUrl }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (sceneRef.current)
                DestroyTongueScene(sceneRef.current);
    ...
    return (<div ref={fullscreenRootRef} className={`tongue-catch tongue-catch-phase-${phase}`}>
      <div ref={jsPsychHostRef} style={{ display: 'none' }} aria-hidden="true"/>
      <video ref={videoRef} className="tongue-catch-video" style={{ opacity: config.cameraOpacity }} muted playsInline aria-label={t('tongue.camera.preview')}/>
      <div ref={pixiHostRef} className="tongue-catch-stage"/>
      <canvas ref={featureCanvasRef} className="tongue-feature-canvas" width={mouthFeatureWidth} height={mouthFeatureHeight} aria-hidden="true"/>

      {null}

      {phase === 'rules' && (<div className="training-panel tongue-menu-panel">
          <MouthTrainingRulesPanel title={t('tongue.title')} summaryTitle={t('tongue.title')} summaryItems={tongueSummaryItems} onStart={() => void startSession()} onBack={showConfiguration}/>
        </div>)}

      {phase === 'initializing' && (<div className="gesture-calibration-overlay">
          <div aria-label={statusMessage || t('tongue.loading.title')} aria-live="polite" className="gesture-loading-card" role="status">
            <div className="gesture-loader" aria-hidden="true"/>
          </div>
        </div>)}

      {phase === 'calibration' && activeCalibrationStep && (<div className="tongue-calibration-overlay">
          ...
          <div className="tongue-calibration-card">
            ...
            <div className="gesture-calibration-actions">
              <button className="btn btn-primary btn-lg" onClick={startCalibrationCapture} disabled={isCapturing}>
      ...
      {phase === 'playing' && !recognition.faceVisible && (<div className="tongue-face-warning">{t('tongue.game.findFace')}</div>)}

      {phase === 'results' && result && (<div className="experiment-container experiment-container-scrollable tongue-results-container">
          <div className="experiment-results">
            ...
            <table className="results-table">
              ...
              <tbody>
                {result.Apple_Results.map((apple, index) => (<tr key={`${index}-${apple.horizontalPositionPercent}`}>
                    <td>{index + 1}</td>
                ...
                {result.Apple_Results.length === 0 && (<tr>
      ...
      {showVisionError && visionError && (<MediaDeviceErrorDialog title={t('tongue.error.title')} titleId="tongue-error-modal-title" message={visionError} onClose={() => setShowVisionError(false)}/>)}
    </div>);
}
function TongueCue({ label }: {
    label: TongueClass;
}) {
    ...
    return (<svg viewBox="0 0 140 100" aria-hidden="true">
      ...
      <rect x={tongueX - tongueWidth / 2} y="50" width={tongueWidth} height="22" rx="11" fill="#fb7185" stroke="#be123c" strokeWidth="3"/>
    </svg>);
}
...
function ResetTongueScene(app: Application, sceneRef: {
    current: TongueScene | null;
}, appleTexture: Texture): void {
    if (sceneRef.current)
        DestroyTongueScene(sceneRef.current);
...
function UpdateTongueGame(args: {
    ...
    const mappedMouth = MapVideoPointToStage(args.mouth.x, args.mouth.y, args.app.screen.width, args.app.screen.height);
    ...
    for (let index = scene.apples.length - 1; index >= 0; index -= 1) {
        ...
        apple.view.x = Clamp(apple.baseX + sway * apple.swayAmplitude, half, args.app.screen.width - half);
...
function TongueIntersectsApple(points: Point[], apple: AppleSprite): boolean {
    ...
    for (let index = 1; index < points.length; index += 1) {
        if (DistanceToSegmentSquared(apple.view.x, apple.view.y, points[index - 1].x, points[index - 1].y, points[index].x, points[index].y) <= collisionRadiusSquared) {
            return true;
        }
    }
    return false;
}
function DistanceToSegmentSquared(pointX: number, pointY: number, startX: number, startY: number, endX: number, endY: number): number {
...
function SpawnApple(scene: TongueScene, width: number, edgeChance: number, resultIndex: number): number {
...
function CloseActiveHold(metrics: SessionMetrics, now: number): void {
    if (metrics.holdStartedAt !== null) {
        ...
        if (duration >= 0.12)
            metrics.holdDurations.push(duration);
    }
    metrics.holdStartedAt = null;
    metrics.holdDirection = null;
}
function CalculateMouthAnchor(landmarks: NormalizedLandmark[]): {
    x: number;
    y: number;
} {
...
function ExtractMouthFeature(video: HTMLVideoElement, canvas: HTMLCanvasElement, landmarks: NormalizedLandmark[]): tf.Tensor {
    const points = lipLandmarkIndices.map((index) => landmarks[index]).filter(Boolean);
    ...
    if (!context)
        return tf.zeros([mouthFeatureWidth * mouthFeatureHeight]);
    context.save();
...
function MapVideoPointToStage(normalizedX: number, normalizedY: number, stageWidth: number, stageHeight: number): {
    x: number;
    y: number;
} {
