...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "依目標位置控制舌頭方向並抓住正確刺激。",
    "en": "Control tongue direction by target position and catch the correct stimulus."
  },
  "stimulus": {
    "zh": "目標會在畫面不同位置出現並移動。",
    "en": "Targets appear and move across different screen positions."
  },
  "response": {
    "zh": "依提示移動或按下對應方向。",
    "en": "Move or press the matching direction for the cue."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:tongue-catch';
  ...
  useEffect(() => {
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-tongue-catch',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="tongue-catch" inert={!complete || undefined} aria-label={"舌頭追蹤 / Tongue Catch"}>
      {children}
    </div>
  );
}
