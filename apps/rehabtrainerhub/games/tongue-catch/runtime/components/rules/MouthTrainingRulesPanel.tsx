...
export function MouthTrainingRulesPanel({ title, summaryTitle, summaryItems, className, onStart, onBack, }: MouthTrainingRulesPanelProps) {
    const { lang } = useT();
    ...
    return (<TrainingRulesPanel className={className} label={isEnglish ? 'Game Rules' : '遊戲規則'} title={title} summaryTitle={summaryTitle} summaryItems={summaryItems} sections={sections} startLabel={isEnglish ? 'Start Training' : '開始訓練'} backLabel={isEnglish ? 'Back to Settings' : '回到設定'} onStart={onStart} onBack={onBack}/>);
}
