...
export function DownloadTrainingCsv({ results, userName, moduleId, t }: any) {
  ...
  const headers = [
    ...
    t('exp.csv.valid'),
    t('exp.csv.collision'),
    t('exp.res.thAvoidance'),
    t('exp.res.thOtherCollision'),
    t('exp.csv.preBrake'),
    t('exp.csv.response'),
  ...
  const rows = (results[0]?.driving_events ?? []).map((event: any) => [
    ...
    event.avoidance_success === null || event.avoidance_success === undefined ? '' : String(event.avoidance_success),
    event.other_vehicle_collision ? 'true' : 'false',
