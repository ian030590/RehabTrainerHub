...
export function PlusMinusGame({ onExit }: PlusMinusGameProps) {
    ...
    const [difficulty, setDifficulty] = useState<string>(GetHostedGameSetting<string>('difficulty'));
    const [problemCount, setProblemCount] = useState<number>(GetHostedGameSetting<number>('problemCount'));
    useEffect(() => {
    ...
    const [phase, setPhase] = useState<PlusMinusPhase>('rules');
    ...
    const startGame = useCallback(() => {
        ...
        try {
            ...
            jsPsychLifecycleRef.current?.start({ moduleId: 'plus-minus', onStart: () => undefined });
        }
        catch {
            // Fallback
        }
        ...
        setTimeout(() => {
            ...
            if (nextProblemIdx < blockTrials.length) {
                ...
                if (nextBlockIdx < blockSequence.length) {
                    ...
                    SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() ?? 'Guest', moduleId: 'plus-minus', gameId: 'plus-minus', gameTitle: document.title, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { module: 'plus-minus',
accuracyPercent,
...
successfulTrials: totalCorrect } });
    ...
    return (<div ref={fullscreenRootRef} className="thinking-game-wrapper" style={{
      ...
      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="plus-minus" onBack={() => RequestHubTrainingConfiguration()} onStart={handleStartPlay} title={document.title}/>)}
      ...
      {phase === 'playing' && (<div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '20px', fontWeight: 800, color: 'var(--accent)' }}>
              {isZh
                ? `階段 ${currentBlockIndex + 1} / 3: ${currentBlockType === 'plus'
                    ? `純加法 (+${offset})`
                    : currentBlockType === 'minus'
                        ? `純減法 (-${offset})`
                        : `交替切換 (+/- ${offset})`}`
                : `Block ${currentBlockIndex + 1} / 3: ${currentBlockType === 'plus'
                    ? 'Pure Addition'
                    : currentBlockType === 'minus'
                        ? 'Pure Subtraction'
                        : 'Alternating Shift'}`}
          ...
          {subPhase === 'block-intro' && (<div style={{
                    width: '100%',
              ...
              <button type="button" onClick={startBlockProblems} style={{
                    minHeight: '64px',
                    width: '100%',
                    maxWidth: '360px',
                    fontSize: '22px',
          ...
          {(subPhase === 'problem' || subPhase === 'feedback') && currentProblem && (<div style={{
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
              ...
              <div style={{
                ...
                {currentProblem.options.map((opt) => {
                    ...
                    return (<button key={opt} type="button" disabled={subPhase === 'feedback'} onClick={() => handleAnswer(opt)} style={{
                            minHeight: '84px',
                            background: btnBg,
      ...
      {phase === 'results' && sessionSummary && (<div style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                justifyContent: 'center',
