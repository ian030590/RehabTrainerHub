...
function GetRuleSections(gameId: string, lang: 'zh' | 'en') {
    ...
    switch (gameId) {
        case 'plus-minus':
            return BuildRules(isZh, '進行純加法、純減法與加減交替運算，測驗心算設定切換的認知靈活性。', 'Perform pure addition, pure subtraction, and alternating switch blocks to assess mental set shifting cost.', ['第一階段：純加法運算（每個數字加上指定數值）。', '第二階段：純減法運算（每個數字減去指定數值）。', '第三階段：加法與減法逐題交替輪流切換運算。', '點選大按鈕快速計算正確答案，結算會顯示切換耗損（Shift Cost）。'], ['Block 1: Pure addition (add offset to each number).', 'Block 2: Pure subtraction (subtract offset from each number).', 'Block 3: Alternating shift (alternate adding and subtracting on each trial).', 'Tap large number buttons to respond quickly; results show mental set shift cost.']);
        default:
