...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "追蹤移動中的目標卡片並指出最後位置。",
    "en": "Track the moving target card and identify its final position."
  },
  "stimulus": {
    "zh": "卡片會在位置之間移動或交換。",
    "en": "Cards move or exchange positions across the board."
  },
  "response": {
    "zh": "記住目標卡並選擇它最後所在的位置。",
    "en": "Remember the target card and select its final position."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:moving-card';
  ...
  useEffect(() => {
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-moving-card',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="moving-card" inert={!complete || undefined} aria-label={"移動卡片 / Moving Card"}>
      {children}
    </div>
  );
}
