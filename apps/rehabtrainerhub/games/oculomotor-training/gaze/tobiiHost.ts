...
export function CreateTobiiValidationTrial(
  ...
  onResult?: (result: ReturnType<typeof ValidationThresholdDeg>) => void,
) {
  ...
  return {
    ...
    on_finish: (data: Record<string, unknown>) => {
      ...
      const result = ValidationThresholdDeg(samples, targets, cssPxPerCm, viewingDistanceCm);
      onResult?.(result);
      ...
      data.validation_passed = result !== null && result.meanErrorDeg <= 3;
    },
  };
}

export function CreateTobiiValidationFlow(cssPxPerCm: number, viewingDistanceCm: number) {
  ...
  const decision = {
    ...
    stimulus: () => {
      ...
      const status = result && result.meanErrorDeg <= 3
        ? '驗證通過／Validation passed'
        : '誤差超過 3° 或有效樣本不足／Error exceeds 3° or too few samples';
    ...
    choices: () => result && result.meanErrorDeg <= 3
      ? ['開始訓練／Start training']
      : ['重新驗證／Revalidate', '繼續訓練／Proceed'],
    data: { eye_tracking_flow_step: 'validation_result', eye_tracking_source: 'tobii' },
    on_finish: (data: Record<string, unknown>) => {
      repeat = !(result && result.meanErrorDeg <= 3) && data.response === 0;
      data.validation_proceeded_after_warning = !repeat && !(result && result.meanErrorDeg <= 3);
    },
  };
  return {
    timeline: { timeline: [validation, decision], loop_function: () => repeat },
    getThresholdDeg: () => result?.thresholdDeg,
    getMeanErrorDeg: () => result?.meanErrorDeg,
  };
}
