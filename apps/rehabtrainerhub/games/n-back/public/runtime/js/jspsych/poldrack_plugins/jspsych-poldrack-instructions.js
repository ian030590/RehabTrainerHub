    ...
    function show_current_page() {
      if (trial.show_clickable_nav) {
        ...
        if (trial.pages.length == 1) {
          nav_html += "<button id='jspsych-instructions-next'>結束說明 / End Instructions</button>"
        } else {
          if (current_page == 0) {
            nav_html += "<button id='jspsych-instructions-next'>下一頁 / Next</button>"
          } else if (current_page == trial.pages.length - 1) {
            if (trial.allow_backward) {
              nav_html += "<button id='jspsych-instructions-back'>上一頁 / Previous</button>";
            }
            nav_html += "<button id='jspsych-instructions-next'>結束說明 / End Instructions</button>"
          } else {
            if (trial.allow_backward) {
