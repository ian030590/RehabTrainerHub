...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "比較目前刺激與前 N 次刺激，持續更新記憶。",
    "en": "Compare the current stimulus with the one N steps back."
  },
  "stimulus": {
    "zh": "刺激依序出現，需保留固定距離前的資訊。",
    "en": "Stimuli appear in sequence; retain the item a fixed number of steps back."
  },
  "response": {
    "zh": "若與 N 次前相同就按下反應鍵。",
    "en": "Press the response when it matches the item N steps earlier."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:n-back';
  ...
  return (
    <div ref={rootRef} data-game-tour="n-back" inert={!complete || undefined} aria-label={"工作記憶更新 / N-back"}>
      {children}
    </div>
  );
}
