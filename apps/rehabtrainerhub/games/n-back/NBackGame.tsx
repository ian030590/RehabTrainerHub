...
export function NBackGame({ onExit }: NBackGameProps) {
  ...
  const [phase, setPhase] = useState<NBackPhase>('menu');
  const [nBackLevel, setNBackLevel] = useState<NBackLevel>(1);
  ...
  const handleTrialTimeout = useCallback(() => {
    if (currentTrialAnsweredRef.current || !activeTrial) return;
  ...
  const handleUserAnswer = useCallback((response: 'match' | 'nomatch') => {
    if (isFixation || currentTrialAnsweredRef.current || !activeTrial) return;
    currentTrialAnsweredRef.current = true;
  ...
  const handleObservationAdvance = useCallback(() => {
    if (isFixation || currentTrialAnsweredRef.current || !activeTrial || !activeTrial.isObservationOnly) return;
    currentTrialAnsweredRef.current = true;
  ...
  return (
    ...
    >
      ...
      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '工作記憶更新（N-back）' : 'Working Memory Updating (N-back)'}
          subtitle={isZh ? '觀察連續出現的圖形，判斷目前圖形是否與「前 N 個」出現的圖形相同。' : 'Continuously maintain and update active items in working memory.'}
        >
          <TrainingConfigSection
            title={isZh ? '記憶步數 (N)' : 'N-back Level'}
            description={isZh ? '設定工作記憶比對的步數距離。' : 'Select how many steps back to compare.'}
          >
            <TrainingConfigOptionGroup
              options={[
                { value: 1, label: isZh ? '1-back（與前 1 個比對）' : '1-back (1 step back)' },
                { value: 2, label: isZh ? '2-back（與前 2 個比對）' : '2-back (2 steps back)' },
                { value: 3, label: isZh ? '3-back（與前 3 個比對）' : '3-back (3 steps back)' },
              ]}
              value={nBackLevel}
              onChange={(val) => setNBackLevel(val as NBackLevel)}
            />
          </TrainingConfigSection>

          <TrainingConfigSection
            title={isZh ? '總題數' : 'Rounds'}
            description={isZh ? '設定本次活動總題數。' : 'Select number of trials in this session.'}
          >
            <TrainingConfigOptionGroup
              options={[
                { value: 15, label: isZh ? '15 題（快速）' : '15 trials' },
                { value: 20, label: isZh ? '20 題（標準）' : '20 trials' },
                { value: 30, label: isZh ? '30 題（挑戰）' : '30 trials' },
              ]}
              value={rounds}
          ...
          <TrainingConfigSection
            title={isZh ? '圖形展示時間' : 'Display Duration'}
            description={isZh ? '每張圖形在畫面停留的時間長度。' : 'Duration each symbol remains visible.'}
          >
            <TrainingConfigOptionGroup
              options={[
                { value: 1500, label: isZh ? '1.5 秒（快速）' : '1.5s (Fast)' },
                { value: 2500, label: isZh ? '2.5 秒（標準）' : '2.5s (Standard)' },
                { value: 3500, label: isZh ? '3.5 秒（從容）' : '3.5s (Relaxed)' },
              ]}
          ...
          <TrainingConfigSection
            ...
            description={isZh ? '作答時是否播放回饋音效。' : 'Play auditory tones on correct/wrong answers.'}
          >
            <TrainingConfigOptionGroup
              options={[
      ...
      {phase === 'rules' && (
        <BrainTrainingRulesPanel
          gameId="n-back"
          title={isZh ? '工作記憶更新（N-back）說明' : 'N-back Working Memory Rules'}
      ...
      {phase === 'playing' && (
        <div style={{
          ...
          <div style={{
            ...
            {activeTrial?.isObservationOnly ? (
              isZh
                ? `請記住此圖形（第 ${currentTrialIndex + 1} / ${nBackLevel} 張記憶觀察）`
                : `Memorize this symbol (${currentTrialIndex + 1} of ${nBackLevel} warmup)`
            ) : (
              isZh
          ...
          <div style={{
            ...
            <div style={{
              ...
              {isFixation ? (
                ...
                <RenderNBackSymbol symbolId={activeTrial.symbolId} size={130} />
              ) : null}
            </div>
          </div>
          ...
          <div style={{ width: '100%', maxWidth: '680px' }}>
            {activeTrial?.isObservationOnly ? (
              <button
                ...
                onClick={handleObservationAdvance}
                style={{
                  width: '100%',
                  minHeight: '84px',
                  display: 'flex',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                <button
                  type="button"
                  disabled={isFixation || currentTrialAnsweredRef.current}
                  onClick={() => handleUserAnswer('match')}
                  style={{
                    minHeight: '88px',
                    display: 'flex',
                    flexDirection: 'column',
                ...
                <button
                  type="button"
                  disabled={isFixation || currentTrialAnsweredRef.current}
                  onClick={() => handleUserAnswer('nomatch')}
                  style={{
                    minHeight: '88px',
                    display: 'flex',
                    flexDirection: 'column',
...
function RenderNBackSymbol({ symbolId, size }: { symbolId: number; size: number }) {
  switch (symbolId) {
    case 1:
      return (
