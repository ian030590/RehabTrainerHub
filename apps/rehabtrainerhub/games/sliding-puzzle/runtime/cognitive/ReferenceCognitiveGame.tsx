...
import { CreateSlidingState,DrawSliding,HandleSlidingTap,IsSlidingAutoSuccess,} from '../../SlidingPuzzleGame';
...
export function ReferenceCognitiveGame({ gameId, onExit, trainingModuleId = 'thinking-training', trainingConfigLabel, }: ReferenceCognitiveGameProps) {
    ...
    const finishGame = useCallback((gameResult: GameResult) => {
        ...
        setResult(record);
        setPhase('results');
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'sliding-puzzle', gameId: 'sliding-puzzle', gameTitle: metaTitle, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { userName: participantId,
...
detailRows: timingData.detailRows } });
    ...
    function HandleCellTap(index: number) {
        ...
        if (state.kind === 'sliding-puzzle')
            HandleSlidingTap(state, index, finishGameRef.current);
        ;
...
function CreateInitialState(gameId: ReferenceGameId, difficulty: Difficulty, reactionTrials: number, simonLives: number): CognitiveGameState { return CreateSlidingState(difficulty); }
...
function IsAutoSuccess(state: CognitiveGameState | null) { if (!state)
    return false; return IsSlidingAutoSuccess(state); }
