...
export function NumberLetterGame({ onExit }: NumberLetterGameProps) {
    ...
    const [phase, setPhase] = useState<NlPhase>('rules');
    useTrainingAbort({ active: phase === 'playing', onAbort: handleExitTraining });
    const [difficulty, setDifficulty] = useState<'alternating' | 'number-only' | 'letter-only'>(GetHostedGameSetting<'alternating' | 'number-only' | 'letter-only'>('difficulty'));
    ...
    const completeSession = useCallback((finalTrials: NumberLetterTrial[]) => {
        ...
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'number-letter', gameId: 'number-letter', gameTitle: isZh ? '數字字母切換 (Number-Letter)' : 'Number-Letter Task Switching', difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { category: 'higher-cognition',
...
notes: isZh
    ...
    const startGame = useCallback(() => {
        ...
        setTrialFeedback(null);
        setPhase('playing');
        jsPsychLifecycleRef.current?.start({ moduleId: "number-letter", onStart: () => undefined });
        startTrialFixation();
    ...
    return (<div ref={fullscreenRootRef} className="cognitive-reference-game" style={{
      ...
      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="number-letter" onStart={() => void startGame()} onBack={() => RequestHubTrainingConfiguration()} title={document.title}/>)}
      ...
      {phase === 'playing' && (<div style={{
          ...
          <div style={{
            ...
            {(['top-left', 'top-right', 'bottom-left', 'bottom-right'] as QuadrantLocation[]).map((loc) => {
                ...
                return (<div key={loc} style={{
                  ...
                  {isActive && activeTrial && (<span style={{
                            fontSize: '44px',
                            fontWeight: 900,
                            color: 'var(--text-primary)',
                            letterSpacing: '3px',
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {activeTrial?.task === 'number' ? (<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <button type="button" onClick={() => handleUserAnswer('odd')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                    minHeight: '76px',
                    fontSize: '22px',
                    fontWeight: 900,
                    borderRadius: '18px',
                ...
                <button type="button" onClick={() => handleUserAnswer('even')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                    minHeight: '76px',
                    fontSize: '22px',
                    fontWeight: 900,
                    borderRadius: '18px',
              ...
              </div>) : (<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <button type="button" onClick={() => handleUserAnswer('vowel')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                    minHeight: '76px',
                    fontSize: '22px',
                    fontWeight: 900,
                    borderRadius: '18px',
                ...
                <button type="button" onClick={() => handleUserAnswer('consonant')} disabled={isFixation || currentTrialAnsweredRef.current} style={{
                    minHeight: '76px',
                    fontSize: '22px',
                    fontWeight: 900,
                    borderRadius: '18px',
...
function GenerateNumberLetterTrials(rounds: number, mode: 'alternating' | 'number-only' | 'letter-only'): NumberLetterTrial[] {
    ...
    for (let i = 0; i < rounds; i++) {
        ...
        if (mode === 'number-only')
            quad = i % 2 === 0 ? 'top-left' : 'top-right';
        else if (mode === 'letter-only')
            quad = i % 2 === 0 ? 'bottom-left' : 'bottom-right';
