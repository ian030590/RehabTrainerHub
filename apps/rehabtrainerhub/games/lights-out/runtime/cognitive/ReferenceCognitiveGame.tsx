...
import { CreateLightsState,DrawLightsOut,HandleLightsTap,IsLightsAutoSuccess,} from '../../LightsOutGame';
...
export function ReferenceCognitiveGame({ gameId, onExit, trainingModuleId = 'thinking-training', trainingConfigLabel, }: ReferenceCognitiveGameProps) {
    ...
    const finishGame = useCallback((gameResult: GameResult) => {
        ...
        setResult(record);
        setPhase('results');
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'lights-out', gameId: 'lights-out', gameTitle: metaTitle, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { userName: participantId,
...
detailRows: timingData.detailRows } });
    ...
    function HandleCellTap(index: number) {
        ...
        if (state.kind === 'lights-out')
            HandleLightsTap(state, index, finishGameRef.current);
        ;
...
function CreateInitialState(gameId: ReferenceGameId, difficulty: Difficulty, reactionTrials: number, simonLives: number): CognitiveGameState { return CreateLightsState(difficulty); }
...
function IsAutoSuccess(state: CognitiveGameState | null) { if (!state)
    return false; return IsLightsAutoSuccess(state); }
function GetFeedbackCounts(state: CognitiveGameState): {
    success: number;
    errors: number;
} { return { success: IsLightsAutoSuccess(state) ? 1 : 0, errors: 0 }; }
