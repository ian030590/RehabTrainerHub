...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "翻轉燈格，讓整個棋盤熄滅。",
    "en": "Toggle the lights until the entire board is off."
  },
  "stimulus": {
    "zh": "按下一格會同時翻轉它與鄰近格子的狀態。",
    "en": "Pressing a cell toggles it and its neighboring cells."
  },
  "response": {
    "zh": "規劃按鍵順序，讓所有燈格變暗。",
    "en": "Plan the press sequence until every light is off."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:lights-out';
  ...
  useEffect(() => {
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-lights-out',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="lights-out" inert={!complete || undefined} aria-label={"燈滅遊戲 / Lights Out"}>
      {children}
    </div>
  );
}
