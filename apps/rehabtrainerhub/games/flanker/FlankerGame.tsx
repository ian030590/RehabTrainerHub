...
export function FlankerGame({ onExit }: FlankerGameProps) {
  ...
  const [phase, setPhase] = useState<FlankerPhase>('menu');
  const [difficulty, setDifficulty] = useState<FlankerDifficulty>('medium');
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      const key = event.key;
      if (key === 'ArrowLeft' || key.toLowerCase() === 'a') handleResponse('left');
      else if (key === 'ArrowRight' || key.toLowerCase() === 'd') handleResponse('right');
    };
  ...
  return (
    <div ref={fullscreenRootRef} className={`cognitive-reference-game flanker-game flanker-phase-${phase}`}>
      <div ref={jsPsychHostRef} style={{ display: 'none' }} />

      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '側翼干擾選擇（Flanker）' : 'Flanker Selective Attention'}
          subtitle={isZh ? '辨識中央箭頭指向並忽略兩側干擾，練習視覺聚焦與干擾抑制。' : 'Identify center arrow direction while ignoring flanking distractors.'}
        ...
        >
          <TrainingConfigSection title={isZh ? '難度設定' : 'Difficulty'}>
            <TrainingConfigOptionGroup
              options={[
                { value: 'easy', label: difficultyLabels.easy, description: isZh ? '60% 同向，30% 中性，干擾較少' : '60% congruent, lower interference' },
                { value: 'medium', label: difficultyLabels.medium, description: isZh ? '同向與反向衝突平衡出現' : 'Balanced congruent and conflict' },
                { value: 'hard', label: difficultyLabels.hard, description: isZh ? '70% 反向衝突，高難度干擾抑制' : '70% conflict trials' },
              ]}
              selectedValue={difficulty}
              onChange={(val) => setDifficulty(val as FlankerDifficulty)}
            />
          </TrainingConfigSection>
          ...
          <TrainingConfigSection title={isZh ? '每題作答時限' : 'Time Limit Per Trial'}>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              {([1200, 2000, 3500, 0] as const).map((ms) => (
                <button
                  key={ms}
                  type="button"
...
function GenerateFlankerTrials(
  difficulty: FlankerDifficulty,
  rounds: number,
): FlankerTrial[] {
