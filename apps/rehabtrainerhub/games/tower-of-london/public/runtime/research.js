(function () {
  ...
  window.rehabConfigure = function(settings, timeline) {
    var value = settings && settings.problemLimitMs;
    if (value === undefined) value = 20000;
    if (!Number.isFinite(value) || value < 10000 || value > 60000 || Math.abs((value - 10000) / 5000 - Math.round((value - 10000) / 5000)) > 1e-8) throw new Error('Invalid problemLimitMs');
    configuredValue = value;
    var questionnaire = timeline.indexOf(window.post_task_block);
    if (questionnaire >= 0) timeline.splice(questionnaire, 1);
    window.time_per_trial = value;
  };
  ...
  window.rehabResearchRows = function(rows) {
    var last = null; return rows.flatMap(function(r) { if (r.exp_stage !== 'practice' && r.exp_stage !== 'test') return []; if (r.trial_id === 'to_board' || r.trial_id === 'to_hand') { last = r; return []; } if (r.trial_id !== 'feedback' || !last) return []; var result = Object.assign(base(r), { rt: null, problemMs: r.problem_time, moves: last.num_moves_made, minimumMoves: last.min_moves, problem: last.problem_id }); last = null; return [result]; });
  };
})();
