...
export function TowerOfLondonGame({ onExit }: TowerOfLondonGameProps) {
    ...
    const [phase, setPhase] = useState<TowerPhase>('rules');
    useTrainingAbort({ active: phase === 'playing', onAbort: handleExitTraining });
    const [difficulty, setDifficulty] = useState<TowerDifficulty>(GetHostedGameSetting<TowerDifficulty>('difficulty'));
    const [rounds, setRounds] = useState(GetHostedGameSetting<number>('rounds'));
    const [showOptimalHint, setShowOptimalHint] = useState(GetHostedGameSetting<boolean>('showOptimalHint'));
    ...
    const completeSession = useCallback((finalTrials: TowerTrialRecord[]) => {
        ...
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'tower-of-london', gameId: 'tower-of-london', gameTitle: isZh ? '倫敦塔規劃 (Tower of London)' : 'Tower of London Planning', difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { category: 'higher-cognition',
...
notes: isZh
    ...
    const handlePegClick = useCallback((pegIndex: number) => {
        if (phase !== 'playing' || isPuzzleSolved || !activePuzzle)
            return;
        if (selectedPeg === null) {
    ...
    const startGame = useCallback(() => {
        ...
        puzzleStartTimeRef.current = performance.now();
        jsPsychLifecycleRef.current?.start({ moduleId: "tower-of-london", onStart: () => undefined });
    ...
    return (<div ref={fullscreenRootRef} className="cognitive-reference-game" style={{
      ...
      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="tower-of-london" onStart={() => void startGame()} onBack={() => RequestHubTrainingConfiguration()} title={document.title}/>)}

      {phase === 'playing' && activePuzzle && (<div style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '17px', fontWeight: 700 }}>
              ...
              {showOptimalHint && (<span style={{ color: 'var(--accent)' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'flex-end', width: '100%', height: '220px', paddingBottom: '16px' }}>
              {[0, 1, 2].map((pegIdx) => {
                ...
                return (<div key={pegIdx} onClick={() => handlePegClick(pegIdx)} style={{
                        display: 'flex',
                        flexDirection: 'column',
                        alignItems: 'center',
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              {[0, 1, 2].map((pegIdx) => {
                const capacity = pegCapacities[pegIdx];
                const isSelected = selectedPeg === pegIdx;
                return (<button key={pegIdx} type="button" onClick={() => handlePegClick(pegIdx)} style={{
                        minHeight: '68px',
                        padding: '8px 4px',
                        fontSize: '18px',
                        fontWeight: 900,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {selectedPeg !== null && (<button type="button" onClick={() => setSelectedPeg(null)} style={{
                    padding: '10px 16px',
                    borderRadius: '12px',
                ...
                <button type="button" onClick={handleResetCurrentPuzzle} style={{
                padding: '10px 16px',
                borderRadius: '12px',
                border: '1px solid var(--border)',
                background: 'var(--card-bg)',
...
function RenderBall({ color, size }: {
    color: BallColor;
    size: number;
}) {
...
function RenderTowerMini({ pegs }: {
    pegs: PegsState;
}) {
    return (<div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px', height: '60px' }}>
      {[0, 1, 2].map((pIdx) => {
            const capacity = pegCapacities[pIdx];
            const balls = pegs[pIdx];
            return (<div key={pIdx} style={{
                    display: 'flex',
                    flexDirection: 'column-reverse',
                    alignItems: 'center',
                    width: '26px',
            ...
            {balls.map((col, bIdx) => (<RenderBall key={bIdx} color={col} size={14}/>))}
          </div>);
        })}
    </div>);
}
    ...
    while (queue.length > 0) {
        ...
        for (let from = 0; from < 3; from++) {
            ...
            for (let to = 0; to < 3; to++) {
                if (from === to)
                    continue;
                if (state[to].length >= pegCapacities[to])
                    continue;
...
function GenerateTowerPuzzles(difficulty: TowerDifficulty, count: number): TowerPuzzle[] {
    ...
    while (puzzles.length < count && attempts < 500) {
        ...
        if (optimal >= minMoves && optimal <= maxMoves) {
            const alreadyExists = puzzles.some((p) => SerializePegs(p.startPegs) === SerializePegs(start) && SerializePegs(p.goalPegs) === SerializePegs(goal));
            if (!alreadyExists) {
                puzzles.push({
