...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "先規劃最少步數，再把球移到目標排列。",
    "en": "Plan the fewest moves, then arrange the balls to match the goal."
  },
  "stimulus": {
    "zh": "起始排列與目標排列會同時顯示，移動受柱槽限制。",
    "en": "Start and goal arrangements appear together with peg limits."
  },
  "response": {
    "zh": "一次移動一顆球，依計畫完成目標排列。",
    "en": "Move one ball at a time and follow your plan to reach the goal."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:tower-of-london';
  ...
  useEffect(() => {
    ...
    const started = StartTour(steps, {
      ...
      storageKey: 'rehab-tour-tower-of-london',
      labels: {
  ...
  return (
    <div ref={rootRef} data-game-tour="tower-of-london" inert={!complete || undefined} aria-label={"倫敦塔規劃 / Tower of London"}>
      {children}
    </div>
  );
}
