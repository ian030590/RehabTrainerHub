...
const copy: GameTourCopy = {
  ...
  "stimulus": {
    "zh": "顏色字的字義可能和墨水顏色衝突。",
    "en": "A colour word may conflict with its ink colour."
  },
  "response": {
    "zh": "只依墨水顏色選擇答案。",
    "en": "Choose based only on the ink colour."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:stroop';
  ...
  useEffect(() => {
    ...
    const target = () => rootRef.current?.querySelector<HTMLElement>("button,iframe") ?? rootRef.current;
  ...
  return (
    <div ref={rootRef} data-game-tour="stroop" inert={!complete || undefined} aria-label={"色彩干擾抑制 / Stroop"}>
      {children}
    </div>
  );
}
