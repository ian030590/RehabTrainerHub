...
export function StroopGame({ onExit }: StroopGameProps) {
    ...
    const [phase, setPhase] = useState<StroopPhase>('rules');
    useTrainingAbort({ active: phase === 'playing', onAbort: handleExitTraining });
    const [difficulty, setDifficulty] = useState<StroopDifficulty>(GetHostedGameSetting<StroopDifficulty>('difficulty'));
    ...
    const startGame = useCallback(async () => {
        ...
        setResults(null);
        setPhase('playing');
        await jsPsychLifecycleRef.current?.start({ moduleId: 'brain:stroop', onStart: () => undefined });
        setIsFixation(true);
        currentTrialAnsweredRef.current = true;
        setTimeout(() => {
            setIsFixation(false);
    ...
    return (<div ref={fullscreenRootRef} className={`cognitive-reference-game stroop-game stroop-phase-${phase}`}>
      <div ref={jsPsychHostRef} style={{ display: 'none' }}/>

      {null}

      {phase === 'rules' && (<BrainTrainingRulesPanel gameId="stroop" title={isZh ? '色彩干擾抑制（Stroop）' : 'Color Word Interference (Stroop)'} summaryTitle={isZh ? '目前設定摘要' : 'Settings Summary'} summaryItems={[
      ...
      {phase === 'playing' && (<div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              {isFixation ? (<span style={{ fontSize: '72px', fontWeight: 900, color: 'var(--text-secondary)' }}>+</span>) : activeTrial ? (<span style={{
                    fontSize: '84px',
                    fontWeight: 900,
                    letterSpacing: '8px',
          ...
          <div style={{
            ...
            {colorOptions.map((opt) => {
                return (<button key={opt.key} type="button" disabled={isFixation || currentTrialAnsweredRef.current} onClick={() => handleResponse(opt.key)} style={{
                        minHeight: '76px',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
...
function GenerateStroopTrials(difficulty: StroopDifficulty, rounds: number, isZh: boolean): StroopTrial[] {
