...
export function AntisaccadeGame({ onExit }: AntisaccadeGameProps) {
  ...
  const [phase, setPhase] = useState<AntiPhase>('menu');
  ...
  const completeSession = useCallback((finalTrials: AntisaccadeTrial[]) => {
    ...
    void SaveTrainingSessionRecord({
      gameId: 'antisaccade',
      gameTitle: isZh ? '反向眼跳抑制 (Antisaccade)' : 'Antisaccade Response Inhibition',
      category: 'attention',
  ...
  const handleOrientationAnswer = useCallback((chosen: TargetOrientation) => {
    if (phase !== 'playing' || trialStep === 'fixation' || trialStep === 'flash' || !activeTrial || trialAnsweredRef.current) return;
  ...
  return (
    ...
    >
      ...
      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '反向眼跳抑制 (Antisaccade)' : 'Antisaccade Task'}
          subtitle={isZh ? '居家注視點反向抑制與反射轉向控制練習' : 'Reflexive visual saccade inhibition activity'}
        >
          <TrainingConfigSection title={isZh ? '閃爍速度' : 'Cue Flash Duration'}>
            <TrainingConfigOptionGroup
        ...
        >
          ...
          >
            ...
            <div style={{ width: '80px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {trialStep === 'flash' && activeTrial?.cueLocation === 'left' && (
                <div
                  style={{
                    width: '36px',
                    height: '36px',
              ...
              {trialStep === 'target' && activeTrial?.targetLocation === 'left' && (
                <span style={{ fontSize: '56px', fontWeight: 900, color: 'var(--accent)' }}>
                  {activeTrial.targetOrientation === 'up' ? '↑' : '↓'}
                </span>
              )}

              {trialStep === 'mask' && activeTrial?.targetLocation === 'left' && (
                <span style={{ fontSize: '48px', fontWeight: 900, color: 'var(--text-secondary)' }}>
                  ■
                </span>
              )}
            </div>
            ...
            {/* Right Slot */}
            <div style={{ width: '80px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {trialStep === 'flash' && activeTrial?.cueLocation === 'right' && (
                <div
                  style={{
                    width: '36px',
                    height: '36px',
              ...
              {trialStep === 'target' && activeTrial?.targetLocation === 'right' && (
                <span style={{ fontSize: '56px', fontWeight: 900, color: 'var(--accent)' }}>
                  {activeTrial.targetOrientation === 'up' ? '↑' : '↓'}
                </span>
              )}

              {trialStep === 'mask' && activeTrial?.targetLocation === 'right' && (
                <span style={{ fontSize: '48px', fontWeight: 900, color: 'var(--text-secondary)' }}>
                  ■
                </span>
              )}
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <button
                type="button"
                onClick={() => handleOrientationAnswer('up')}
                disabled={trialStep === 'fixation' || trialStep === 'flash' || trialAnsweredRef.current}
                style={{
                  minHeight: '76px',
                  fontSize: '24px',
                  fontWeight: 900,
              ...
              <button
                type="button"
                onClick={() => handleOrientationAnswer('down')}
                disabled={trialStep === 'fixation' || trialStep === 'flash' || trialAnsweredRef.current}
                style={{
                  minHeight: '76px',
                  fontSize: '24px',
