...
export function ReferenceCognitiveGame({ gameId, onExit, trainingModuleId = 'thinking-training', trainingConfigLabel, }: ReferenceCognitiveGameProps) {
    ...
    const finishGame = useCallback((gameResult: GameResult) => {
        ...
        setResult(record);
        setPhase('results');
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'hex', gameId: 'hex', gameTitle: metaTitle, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { userName: participantId,
