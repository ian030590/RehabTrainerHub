...
export function GoNoGoGame({ onExit }: GoNoGoGameProps) {
  ...
  const [phase, setPhase] = useState<GoNoGoPhase>('menu');
  const [difficulty, setDifficulty] = useState<GoNoGoDifficulty>('medium');
  const [rounds, setRounds] = useState(30);
  const [stimulusDurationMs, setStimulusDurationMs] = useState(1200);
  ...
  return (
    <div ref={fullscreenRootRef} className={`cognitive-reference-game gonogo-game gonogo-phase-${phase}`}>
      <div ref={jsPsychHostRef} style={{ display: 'none' }} />

      {phase === 'menu' && (
        <TrainingConfigPanel
          title={isZh ? '通行與抑制反應（Go / No-go）' : 'Go / No-Go Sustained Attention'}
          subtitle={isZh ? '看到綠色通行訊號迅速點擊，看到紅色禁止訊號抑制點擊。' : 'Respond quickly to green Go signals; suppress responses on red No-Go signals.'}
        ...
        >
          <TrainingConfigSection title={isZh ? '難度設定' : 'Difficulty'}>
            <TrainingConfigOptionGroup
              options={[
                { value: 'easy', label: difficultyLabels.easy, description: isZh ? '80% 通行訊號，刺激停留時間寬鬆' : '80% Go, relaxed duration' },
                { value: 'medium', label: difficultyLabels.medium, description: isZh ? '75% 通行訊號，標準節奏' : '75% Go, standard pace' },
                { value: 'hard', label: difficultyLabels.hard, description: isZh ? '70% 通行訊號，快速切換考驗衝動抑制' : '70% Go, fast switching' },
              ]}
              selectedValue={difficulty}
              onChange={(val) => setDifficulty(val as GoNoGoDifficulty)}
            />
          </TrainingConfigSection>
          ...
          <TrainingConfigSection title={isZh ? '刺激停留時限' : 'Display Duration'}>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              {([800, 1200, 1600] as const).map((ms) => (
                <button
                  key={ms}
                  type="button"
...
function GenerateGoNoGoTrials(difficulty: GoNoGoDifficulty, rounds: number): GoNoGoTrial[] {
  ...
  let goRatio = 0.75;
  if (difficulty === 'easy') goRatio = 0.8;
  else if (difficulty === 'hard') goRatio = 0.7;
