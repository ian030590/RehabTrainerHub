...
const copy: GameTourCopy = {
  ...
  "goal": {
    "zh": "看到通行刺激要反應，看到禁止刺激要抑制反應。",
    "en": "Respond to go stimuli and withhold responses to no-go stimuli."
  },
  "stimulus": {
    "zh": "刺激會在通行與禁止條件間切換。",
    "en": "Stimuli alternate between go and no-go conditions."
  },
  "response": {
    "zh": "通行時按鍵，禁止時保持不動。",
    "en": "Press for go trials and stay still for no-go trials."
  }
};
...
export function GameTour({ children }: { children: ReactNode }) {
  ...
  const completionEvent = 'rehab-trainer:game-tour-complete:go-nogo';
  ...
  return (
    <div ref={rootRef} data-game-tour="go-nogo" inert={!complete || undefined} aria-label={"通行與抑制反應 / Go / No-Go"}>
      {children}
    </div>
  );
}
