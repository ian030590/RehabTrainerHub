...
import { CreateMemoryState,DrawMemory,HandleMemoryTap,IsMemoryAutoSuccess,UpdateMemoryTimedState,} from '../../MemoryMatchGame';
...
export function ReferenceCognitiveGame({ gameId, onExit, trainingModuleId = 'thinking-training', trainingConfigLabel, }: ReferenceCognitiveGameProps) {
    ...
    const finishGame = useCallback((gameResult: GameResult) => {
        ...
        setResult(record);
        setPhase('results');
        void SaveTrainingSessionRecord({ userName: GetAuthUserNameFromToken() || 'Guest', moduleId: 'memory-match', gameId: 'memory-match', gameTitle: metaTitle, difficulty: 'configured', trainingDate: FormatTestDate(new Date()), details: { userName: participantId,
...
detailRows: timingData.detailRows } });
    ...
    function HandleCellTap(index: number) {
        ...
        if (state.kind === 'memory-match')
            HandleMemoryTap(state, index, metricsRef.current.elapsed, finishGameRef.current);
        ;
...
function CreateInitialState(gameId: ReferenceGameId, difficulty: Difficulty, reactionTrials: number, simonLives: number): CognitiveGameState { return CreateMemoryState(difficulty); }
function UpdateTimedState(state: CognitiveGameState | null, elapsed: number, render: () => void, finishGame: (result: GameResult) => void, onSimonInputStart: () => void) {
    if (!state)
        return;
    if (state.kind === 'memory-match')
        UpdateMemoryTimedState(state, elapsed, render);
    ;
}
function IsAutoSuccess(state: CognitiveGameState | null) { if (!state)
    return false; return IsMemoryAutoSuccess(state); }
function GetFeedbackCounts(state: CognitiveGameState): {
    success: number;
    errors: number;
} { return { success: state.matchedPairs, errors: state.errors }; }
