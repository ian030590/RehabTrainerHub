...
export function GestureBattlerGame({ onExit }: GestureBattlerGameProps) {
    ...
    const handleCombatHand = useCallback((landmarks: NormalizedLandmark[], now: number) => {
        ...
        const isEligible = Boolean(match.gesture &&
        ...
        if (progress >= 1)
            void triggerAttack(match.gesture, match.similarity);
    ...
    const handleCalibrationHand = useCallback((landmarks: NormalizedLandmark[], now: number) => {
        ...
        if (!step)
            return;
        setHandVisible(true);
        if (!calibrationCapturingRef.current)
            return;
        ...
        if (calibrationSamplesRef.current.length >= calibrationMinStableSamples) {
            ...
            if (CalibrationFeatureDifference(previousFeatures, rawFeatures) >= calibrationChangeThreshold) {
                advanceCalibration(previousFeatures, step, t('gesture.calibration.changeStopped'));
                return;
            }
        }
    ...
    const handleNoHand = useCallback((now: number) => {
        setHandVisible(false);
        setCombatGesture(null);
        if (now - lastHandSeenAtRef.current <= trackingGraceMs)
            return;
        if (phaseRef.current === 'calibration') {
            ...
            if (step && calibrationSamplesRef.current.length >= calibrationMinStableSamples) {
                advanceCalibration(AverageVectors(calibrationSamplesRef.current), step, t('gesture.calibration.changeStopped'));
                return;
            }
            calibrationCapturingRef.current = false;
            calibrationHoldStartRef.current = null;
    ...
    useEffect(() => {
        ...
        const initialize = async () => {
            ...
            app.ticker.add((ticker: Ticker) => {
                ...
                if (!scene || phaseRef.current !== 'combat')
                    return;
    ...
    return (<div ref={fullscreenRootRef} className={`gesture-battler gesture-battler-phase-${phase}`}>
            ...
            <div className="gesture-move-list">
              {gestures.map((gesture) => {
                ...
                return (<div key={gesture} className={`gesture-move ${selected ? 'selected' : ''} ${eligible ? '' : 'disabled'}`.trim()}>
                    ...
                    {selected && eligible && (<i className="gesture-cast-ring" style={{ '--gesture-progress': `${combatHoldProgress * 360}deg` } as React.CSSProperties} aria-hidden="true"/>)}
                  </div>);
            })}
            </div>
          </section>
      ...
      {phase === 'calibration' && activeCalibrationStep && (<div className="gesture-calibration-overlay">
          <div className="gesture-calibration-card">
            <div className="gesture-calibration-copy">
              ...
              {calibrationNotice && (<span className="gesture-calibration-notice">{calibrationNotice}</span>)}
            </div>
            <GestureCue gesture={activeCalibrationStep.gesture}/>
            <div className="gesture-calibration-actions">
              <button className="btn btn-primary btn-lg" type="button" onClick={startCurrentCalibration} disabled={isCalibrationCapturing}>
                {isCalibrationCapturing
      ...
      {phase === 'results' && result && (<div className="experiment-container experiment-container-scrollable gesture-results-container">
          <div className="experiment-results">
            ...
            <table className="results-table">
              ...
              <tbody>
                {resultRows.map((stat) => (<tr key={stat.Gesture}>
                    <td>{stat.Gesture}</td>
                    <td>{stat.Attempts}</td>
...
function GestureCue({ gesture }: {
    ...
    return (<div className="gesture-cue" aria-hidden="true">
      <svg viewBox="0 0 180 220">
        ...
        {[0, 1, 2, 3].map((finger) => {
            ...
            return (<rect key={finger} className={extended ? 'extended' : ''} x={x} y={95 - height} width="18" height={height} rx="9"/>);
        })}
...
function ClassifyGesture(features: number[], profiles: Partial<Record<GestureId, number[]>>): {
    gesture: GestureId | null;
    similarity: number;
} {
    ...
    gestures.forEach((gesture) => {
        ...
        if (!profile || profile.length !== features.length)
            return;
...
function JointAngle(first: NormalizedLandmark, middle: NormalizedLandmark, last: NormalizedLandmark): number {
...
function AnimateAttack(app: Application | null, scene: BattleScene | null, gesture: GestureId): Promise<void> {
    if (!app || !scene)
