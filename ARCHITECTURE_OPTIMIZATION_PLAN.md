...
狀態：Recovery / In progress（`f67977d` 是使用者確認的最後可運行基線；reset 後工作樹仍包含後續架構切片。2026-09-01 已以 exact Node/pnpm 完成 frozen install、Hub production build、startability state、entrypoint 與 registry browser acceptance；Git metadata 仍非 canonical，尚不能把整個工作樹視為已完成 recovery。文件中的既有實作勾選表示工作樹中已有對應程式，不等同於已完成所有架構 phase。）

盤點日期：2026-09-01

適用範圍：Hub 內建訓練、開發者遊戲平台、隔離執行器、PWA、CI/CD 與套件管理；除本輪生命週期框架與可運行驗收規範外，其餘架構決策不變
...
| 議題 | 建議決策 |
| --- | --- |
| 是否直接刪除 mouth / motor / vision / brain runtime | **不要直接刪除 runtime 邊界。**逐步退役四個「類別型 Vite shell」，改成每局建立 fresh iframe 的輕量 `official-training-host`，再由每個遊戲動態載入並擁有自己的 engine/run handle。 |
...
| 何時開始載入 Pixi / Three / MediaPipe / TensorFlow / WebGazer | config 與規則 UI 不得靜態 import 重型依賴。進入 `rules-visible` 後才允許下載程式碼與資產；使用者按「開始」後才允許建立 run handle、renderer、model instance、實際 media stream，以及模組若有採用的 jsPsych instance。 |
| 生命週期框架規範 | 新遊戲必須由 module-owned engine 實作統一的 `TrainingRunHandle`，但可自行選擇 native/custom engine、component-owned game loop 或 jsPsych plugin/timeline；**jsPsych 不是必要條件**。既有 external adapter 仍只是遷移橋接，不是新模組範本。 |
...
| 單一 HTML/ZIP 的小 CI | 保留現有同步 intake scanner，新增 queue 驅動的隔離掃描服務：結構檢查、AST/HTML policy、malware signature、Game SDK/lifecycle contract、無網路動態 smoke；只有宣告使用 jsPsych 的套件才驗證其 pinned runtime 相容性。掃描完成才進管理後台審核佇列。 |
...
| 可運行基線與合併門檻 | `f67977d` 保留為 last-known-runnable recovery anchor。後續工作不得只靠 source token、型別或 build 判定完成；每個 official-host 垂直切片都必須通過 loading 狀態轉移檢查與真實瀏覽器 config → rules-ready → running 驗收，並至少驗證一次 fresh 第二局。 |
...
明確排除本次 10 模組 lifecycle contract 驗收切片：

- `hart-chart`（哈特圖）
- `driving-rehab`（駕駛注意力模擬練習）

排除代表「不要求在本工作流轉換」，不是允許它們破壞既有 lifecycle、PWA 或 entrypoint gate。以上 10 個模組與未來新增遊戲都必須遵守相同的 host/run-handle lifecycle contract，但不要求以 jsPsych 實作。
...
- `training-modules/moduleFlowManifest.ts` 已對所有 catalog 遊戲宣告 flow、媒體權限與目前的 jsPsych lifecycle 類型；這是既有實作 metadata，不是目標架構對框架的永久限制。
...
- 上傳端已有檔案/解壓大小、路徑、混淆、危險 API、jsPsych/SDK contract 與 SHA-256 檢查；目前 scanner 仍把 jsPsych 當必要條件，需在 Phase 4 改成通用 Game SDK lifecycle contract。
- 發布端已有 quarantine、人工三項確認、lease、核准前重新驗證檔案雜湊、immutable release 與 revoke 流程。

### 2.2 現況同時存在 jsPsych 與 component-owned engine

| 範圍 | 現況 | 目標 |
| --- | --- | --- |
| Vision：moving / oculomotor / gabor / reading | `native-timeline` | 可保留既有 jsPsych timeline，補強 framework-neutral run-handle 行為測試與 loading boundary |
| Brain：UFOV | `native-timeline` | 可保留既有 jsPsych timeline，將 config/rules 與 engine chunk 明確切開 |
| Motor 四個遊戲 | module-owned jsPsych lifecycle scaffold；實際 renderer/input 由 component 擁有；Hub 已接入 native host setup/engine | 保持 component-owned renderer/input，以統一 run handle 對接 host；不必為框架一致性增加或搬移 jsPsych instance |
| Mouth：tongue-catch | module-owned jsPsych lifecycle scaffold；實際 renderer/model/stream 由 component 擁有；Hub 已接入 native host setup/engine | 保持 component-owned renderer/model/stream，以統一 run handle 對接 host；不必為框架一致性增加或搬移 jsPsych instance |
| Hart Chart（排除） | `external-runtime-adapter` | 本輪不轉換，維持回歸測試 |
| Driving（排除） | `native-timeline` | 本輪不轉換，維持 Three/fullscreen 回歸測試 |

目前 manifest、架構測試與上傳 scanner 的確把 jsPsych 寫成必要條件，但實際遊戲的 renderer、input、timer、model 與 stream 多半仍由各自 component/engine 管理；共通且已可使用的邊界是 `TrainingRunHandle`，第三方 Game SDK 也已有 framework-neutral 的 `CreateTrainerHubGameBridge()`。因此下一個垂直切片應移除「必須由 jsPsych 控制生命週期」的 schema/gate，改驗證 module-owned run handle 或 Game SDK bridge 的 start/pause/resume/abort/complete/dispose 行為；既有 jsPsych timeline/plugin 可原樣保留，不需要建立第二個 jsPsych instance 來證明 host migration。
...
- 沒有無網路、無 secret 的通用 Game SDK lifecycle smoke；既有 jsPsych helper 也沒有獨立的 adapter smoke。
...
目前工作樹已有 `packageManager: pnpm@11.24.0`、`pnpm-workspace.yaml`、`pnpm-lock.yaml`、`.node-version`、`workspace:*`，root scripts、兩份 GitHub workflow 與主要文件也已改用 pnpm。已使用 `.node-version` 指定的 Node `v24.20.0` 完成 `pnpm install --frozen-lockfile`，並通過 `test:pnpm-policy`、Hub TypeScript、`build:hub` 與 official browser acceptance。

- 本機驗收使用 pnpm 11.24.0、isolated linker 與 workspace symlink；不得改回 npm 或混用 lockfile。
- 已恢復完整 TypeScript toolchain，不能再以缺失的 dependency 跳過 typecheck 或 browser gate。
- 尚未在 canonical Git recovery checkout 執行所有 gate 與 Cloudflare build；這是 Git metadata/recovery 工作，不可由目前通過的 build 取代。

即使 local frozen install、build 與 browser acceptance 已完成，Phase 0 仍須在 canonical recovery checkout 重跑，才可標記整體 recovery 可部署。

### 2.8 Reset 後工作樹與「遊戲不能開始」的實際風險

Git reflog 顯示 `f67977d chore: unify component ui` 之後曾依序建立 `c5a25e1`、`dd032c0`、`3d26df0`、`c11827a`、`e2774ea`、`1c7bffe`；之後由 `3d26df0` reset 回 `f67977d`。依使用者確認，`f67977d` 是最後可運行版本；後續 commit 內容只能視為待重新驗收的候選變更。

目前工作樹不是乾淨的 `f67977d`：

- pnpm、official host、module setup/engine、PWA、validator 等後續切片仍存在於工作樹；不能把它們整批視為已接受，也不能只因檔案已存在就保留 `[x]` 的可運行含義。
- Git canonical `.git/index` 與 `refs/heads/main` 不存在，卻有 `index [conflicted]`、`refs/heads/main [conflicted]` 與 `logs/refs/heads/main [conflicted]`；因此 `git status` 顯示「No commits yet」及全工作樹 untracked。修復 Git metadata 前不得 commit、rebase、判讀一般 diff 或改寫 recovery anchor。
- Dependency-free 的 `check-official-training-host.mjs` 與 `check-heavy-load-boundary.mjs` 可以通過，但這些 gate 只證明 import/token 邊界，沒有證明使用者能按下開始。
- `NativeTrainingSurface.ensureEngine()` 曾在成功預載後設定 `engineRef.current`，卻沒有把 phase 從 `loading` 恢復成可開始的 `rules`；同一畫面的 Start 又在 `phase === 'loading'` 時 disabled。此問題已修正，並由 `test:training-startability-state` 與 browser acceptance 驗證成功 preload 回到 enabled Start。
- `reading-training` 曾使用 Vite-only `import.meta.glob` 載入故事。Hub 的 Next/Turbopack build 會把該 glob 輸出為空集合，使 `getRandomStory()` 回傳 `null`、timeline 變成 `[]` 並被 jsPsych 誤記為 completed。故事資料現改為明確 JSON imports；`BuildReadingTimeline()` 缺少必要內容時必須 throw，絕不可把空 timeline 視為完成。這是跨 bundler module data 的通用規範，不是 jsPsych 的必要條件。

這些失效與遊戲是否採用 jsPsych 無關；它們證明平台真正需要的是 framework-neutral 的可啟動性與 run-handle 驗收。快速 state gate 與 browser gate 必須持續從真實 UI 證明每個 registry module 進入 stable running，而非只完成 build、或在 first paint 前直接落入 results；在兩者通過前不得再稱該切片「可運行」。
...
│           └─ module-owned engine（每次 run 新建）
│              ├─ optional jsPsych instance + plugin/timeline
│              ├─ custom/component-owned game loop
...
└─ separate registrable domain: usergamerunner
   ├─ reviewed runtime（pinned game SDK；jsPsych 為可選平台 runtime）
...
packages/
  ...
  game-sdk/                      # 第三方遊戲私有 bridge；另提供可選 jsPsych helper
...
export interface TrainingModuleManifest {
  ...
  lifecycle: {
    owner: 'module';
    mode: 'native-run-handle' | 'legacy-adapter-exempt';
  };
  pwa: {
    installable: boolean;
    shortNameKey: string;
...
本節刻意分層：`ValidationIssue`、manifest、result 與純資料 state machine 放在 dependency-free 的 `packages/training-contracts`；含 React type 的 setup adapter 放在 `packages/ui`；run handle 由 module engine 實作。Host 永遠看不到 timeline、jsPsych instance、renderer 或其他 framework-specific state。
...
```ts
// framework-neutral module engine contract；實作可自行選擇是否 import jsPsych
...
- `startRun()` 由模組建立全新的當局 engine state 並回傳 `TrainingRunHandle`。採用 jsPsych 的模組可在內部建立 instance/timeline，並在 plugin `trial()` 初始化 renderer/media；custom engine 則以自己的 component/game loop 管理相同資源。
- Host 只呼叫 `startRun/pause/resume/abort/dispose`，不得 import 或操作 `initJsPsych`、jsPsych data store、custom game loop、renderer 或任何 framework-specific state。
...
- 每個 async phase 都必須明確定義 pending、success、failure 與 abort transition；不得讓 `loading`、`starting` 或 disabled action 成為沒有出口的狀態。
- `loadEngine()` 成功掛上當局 engine 後，host 必須進入 `rules-ready`（目前 UI diagnostic 對應 `data-phase="rules"`）並啟用 Start；失敗或 abort 則回到可重試／可返回的狀態並顯示原因。
- 使用者按 Start 後，在 bounded timeout 內只能得到兩種結果：進入 `running` 且 training mount 有實際內容，或收到明確、可恢復的錯誤；不得靜默停在 loading、空白 iframe 或 disabled button。
- 關閉、abort、完成或錯誤後必須移除當局 realm。至少對一個模組連續建立 fresh 第二局，證明前一局的 promise、phase、renderer、listener、media 與 module singleton 沒有污染下一局。
- Source token、型別檢查、import graph 與 build 只能作快速前置 gate，不能取代瀏覽器從 Hub launch action 走到第一個可觀察 training frame 的驗收。

媒體權限可在 config 做 permission preflight，但取得的測試 stream 必須立即停止；不得藉此提早載入 model。正式 stream 只能在收到開始命令並進入 `startRun()` 後由 module engine 建立與釋放；採用 jsPsych 的模組可把此邊界放在 trial start。
...
Iframe 內的 host adapter 只把 `start/pause/resume/abort/dispose` 映射到當次 module `TrainingRunHandle`；jsPsych object（若有）、custom engine state、renderer reference 與 data store 永不跨訊息邊界。`prepare.config` 必須先以 module schema 驗證，未知欄位剔除後才交給 engine。
...
`path` 只能是 runner root-relative、platform-generated 的固定版本路徑。Game SDK 是所有第三方遊戲的必要生命週期橋接；jsPsych 只有在 artifact 明確採用時才加入 release manifest，且仍只能使用平台提供並驗證過的 pinned runtime。Uploaded game capability 明確不含 network、camera、microphone、worker、storage 或 navigation；Hub 與 runner 必須引用同一份 capability enum 與 runtime contract，消除目前兩邊 allowlist 不一致的風險。
...
- content-addressed Pixi/Three/MediaPipe/WASM vendor chunk，以及採用 jsPsych 的模組所需之 pinned jsPsych chunk
- 版本化 model bytes
- renderer-independent TypeScript contract、數學 helper、UI 與 asset resolver

禁止共用：

- Pixi `Application`、Three scene/renderer、custom engine state，以及模組若有採用的 jsPsych instance
...
5. **Lifecycle conformance**：exact pinned Game SDK URL、`CreateTrainerHubGameBridge()` lifecycle/result schema、asset allowlist、license/SBOM。若套件宣告使用 jsPsych，再額外驗證 exact pinned jsPsych URL、`RunTrainerHubJsPsychGame()` 與 non-empty timeline；不得把這些 jsPsych-specific 檢查套到 custom engine。
...
| Game SDK、可選 jsPsych runtime version 與 URL | build 產生的 platform runtime contract | lockfile/package version/hash parity；未宣告 jsPsych 的遊戲不得被要求載入它 |
...
優先處理目前明顯重複：四類 trainer array、release status unions、Game SDK／可選 jsPsych URL、result limits、runtime IDs、catalog/PWA mapping，以及 `ModulePage.tsx` 中的 inline 中英文字串。
...
- [x] 工作樹已加入第 8 節的 package manager、CI/CD 與文件切換；仍待下列 recovery 驗收。
- [ ] 修復 canonical Git `HEAD/index/refs`，移除 metadata 與工作樹中的 sync-conflict filename，並先通過 `test:repository-health`。修復前不得建立 commit。
- [x] 在本文件記錄 `f67977d` 為使用者確認的 last-known-runnable recovery anchor。
- [x] 使用 `.node-version` 的 exact Node 與 `pnpm@11.24.0` 完成 frozen install；不得沿用缺 TypeScript 的半成品 `node_modules`。
- [ ] 從 coherent `f67977d` checkout 比對並以可驗收垂直切片重新引入後續變更；每片都保留 source gate、build 與 browser acceptance 結果。
- [ ] 在乾淨 checkout 執行 frozen install、全部現有 gates、official runtime browser gate 與 Cloudflare build。
...
驗收：`HEAD` 可解析到預期 commit、canonical index/refs 無 conflict artifact；repo 中除歷史說明外沒有 install/build 指令使用 npm；CI 與本機只接受 `pnpm-lock.yaml`，且乾淨 checkout 能完成 frozen install 與全部 gate。
...
- [x] 把 game capability、jsPsych/SDK runtime version、上傳大小上限從 scanner/runner/developer UI 的複本移至 renderer-independent contract；trainer IDs 與 PWA closure 仍待後續切片。現行 contract 仍把 jsPsych 設為必填，改成 Game SDK 必填、jsPsych 可選列入 Phase 4 待辦。
...
- [x] `test:training-startability-state` 已通過，證明成功 preload 一定離開 loading；`test:official-training-runtime-browser` 已以 production Hub 對 registry 中每個 native setup 完成 config → rules-ready → stable running，並對第一個模組完成 fresh 第二局。閱讀訓練也必須通過此 gate，不得因空資料直接進 results。

驗收：依賴圖只有 `host -> module -> contracts/ui`；module source 不再引用 `training-runtimes`；host entry 不含 `initJsPsych` 或其他 framework-specific engine import；所有已登錄 native setup 都能由真實 Hub UI 啟動，且連續兩局的 `Window/engine/renderer` identity 全部不同（採用 jsPsych 時其 instance identity 也必須不同）；未宣告 camera/microphone/fullscreen 的 iframe browser test 均被拒絕。

### Phase 3：Rules-visible loading boundary

- 本輪已先完成 Vision vertical slice：`CreateSingleFlightPreloadCache` 將同一 module 的 rules-visible preload 合併為單一 promise，離開/換模組時以 `AbortController` 清理 Pixi warmup；WebGazer script loader 也支援 abort 與 script node cleanup。四個 Vite runtime 現在輸出 `.vite/manifest.json`，Hub build 會執行 `check-runtime-build-manifest.mjs`，驗證 root entry 的 static closure 不含 heavy chunk。10 個目標模組與 Every Ball 已建立 module-owned setup/engine boundary；component 內既有 jsPsych 實例可繼續作為模組內部實作，不因 host migration 強制搬移，brain legacy adapter 與瀏覽器 network test 仍待後續收斂。
...
- [ ] 執行 browser network test：規則前不得請求 heavy chunks/models。（`scripts/check-official-game-pwa-browser.mjs` 已提供部署後 network gate；目前機器雖有 Chromium-based browser，但 Git/toolchain 不一致且無法產生目前工作樹的乾淨 build，既有 stale output 不得當作驗收。）
- [x] camera/microphone preflight 後立即 stop；實際 stream 只在 `startRun()` 後存活，採用 jsPsych 的既有模組目前在 trial start 建立（共用 preflight 與 `test:media-lifecycle`）。

驗收：除使用者明確發起、只寫 cache 的 offline pack download 外，10 個目標遊戲在 card/config 階段都沒有任何 heavy dependency（包含模組若採用的 jsPsych、Pixi、Three、MediaPipe、TF、WebGazer）network request、module execution 或 instance。

### Phase 4：framework-neutral lifecycle contract 收斂

- `drawing-defense`、`gesture-battler`、`motor-cortex-rehab`、`asteroid-shield` 與 `tongue-catch` 已完成既有 jsPsych native lifecycle scaffold；plugin 只呼叫 module-owned `on_start` 並以 `finishTrial`/`abortExperiment` 由 component 結束，不保存 Pixi、pointer、model、stream 或 game state。這些已完成且受測的實作可保留，但不再定義新遊戲的必要框架；其餘認知模組仍維持相容 adapter。

- [x] 保留五個既有影音模組的 jsPsych plugin scaffold 與 start/error/stale-run 回歸測試，不保存 renderer/model/stream/game state。
- [x] 保留 Vision 四遊戲與 UFOV 現有 native timeline，繼續透過統一 contract/summarizer 產生 `TrainingRunResult`；不為移除強制條件而重寫已正常工作的 timeline。
- [x] 保留現有 jsPsych plugin 的 data-only simulation，讓其 trial/result schema 可在 CI 快速驗證（不啟動 renderer、模型或媒體）；此 simulation 只測該實作，不是所有遊戲的驗收格式。
- [ ] 將 `TrainingModuleManifest.lifecycle.owner: 'jspsych'`、`jsPsychLifecycle` 必填欄位與 `test:architecture` 的 native-timeline 強制 gate，遷移成 `owner: 'module'` 的 `native-run-handle` contract；framework 選擇留在 module engine 內部，不成為平台資格 gate。
- [ ] 為至少一個不使用 jsPsych 的 official module/fixture 加入 start/pause/resume/abort/finish/dispose 行為測試，證明 host contract 不依賴特定 framework。
- [ ] 將第三方 intake scanner、submission/release schema、runner 與範例由 `RunTrainerHubJsPsychGame()` 必填改成 `CreateTrainerHubGameBridge()` 必填；jsPsych runtime/helper 只在套件選用時驗證並發布。
- [ ] Hart/Driving 的 exemption metadata 改作 legacy migration 記錄；framework-neutral contract 完成後，不再需要以「未轉 native jsPsych」作為 exemption 理由。

驗收：目標 10 個模組都由 module-owned engine 暴露相同 `TrainingRunHandle`，並通過 start/pause/resume/abort/finish/dispose 行為測試；選擇 jsPsych 或 custom engine 都能通過，且任何 manifest、scanner、runner 或 CI gate 都不再要求新遊戲必須使用 jsPsych。External adapter 只剩有移除期限/owner 的既有 legacy modules。
...
pnpm run test:repository-health
pnpm run test:training-flow
pnpm run test:assessment-lifecycle
pnpm run test:entrypoints
pnpm run test:media-lifecycle
pnpm run test:training-startability-state
pnpm run test:official-training-runtime-browser # includes pnpm run build:hub
...
- `test:repository-health`：`HEAD^{commit}`、canonical `.git/index`、refs/logs 與 repository filename 一致；任何 `[conflicted]`/conflict-copy Git metadata、tracked filename 或未被 ignore 的 filename 都直接失敗。這個 gate 防止在實際基線不可解析時把全工作樹誤當成一般 uncommitted diff
...
- `test:training-startability-state`：dependency-free 快速 gate；鎖定 engine preload 成功路徑必須由 `loading` 回到可開始的 rules state，並保留 browser diagnostic
- `test:official-training-runtime-browser`：先 build Hub，再從 setup registry 自動取得所有 module IDs，以 Chromium 逐一操作 Hub launch、config、rules、enabled Start 與 stable running mount；第一個 module 再以 fresh navigation/iframe 執行第二局，並拒絕 runtime exception、關鍵 Document/Script/Stylesheet 載入錯誤，以及開始後在觀察到 running 前直接進入 results 的假完成。任何同時由 Next/Turbopack 與 Vite 執行的 module data loader 都必須使用兩者可解析的 import 方式，且必填資料缺失時必須 fail/abort，不能產生空 timeline。
...
- `test:training-lifecycle`：framework-neutral module run handle 與 shared component training engine 的 start/pause/resume/abort/finish/dispose 行為；保留五個既有 jsPsych plugin 與 adapter 的專屬回歸案例，但不得以其 token/API presence 作為所有新遊戲的 gate。Component setup 的型別、heavy-import 與 host callback 邊界由 `test:training-contracts` 與 `test:heavy-load-boundary` 鎖定
...
- `test:architecture`：module/runtime ownership、framework-neutral lifecycle contract（jsPsych 僅可為 module 內部的可選 implementation）、official/third-party iframe policy、PWA scope、game validation 狀態軸與 pnpm 安裝不變量

`test:architecture` 是不依賴網路與 build output 的靜態架構 gate；它允許清單內既有
legacy adapter 以維持遷移相容性，但禁止新模組加入 adapter；新模組直接實作
`TrainingRunHandle`，並可自行選擇 custom engine 或 jsPsych。它不把同步 intake scanner
當成完整 malware CI，也不取代 Phase 3/4 的 heavy dependency graph、offline closure
與 framework-neutral lifecycle 行為測試。

三個 recovery/startability gate 必須以完全相同命令列入 `.github/workflows/ci.yml` 與 `.github/workflows/deploy-cloudflare-pages.yml` 的 verify matrix。瀏覽器 job 可與其他 gate 平行，但 deployment 必須等它成功；不得因 source/build gate 已綠燈而略過。

Asteroid Shield、fullscreen 或 Pixi canvas 尺寸變更仍至少執行 `test:entrypoints` 與 `build:hub`。Official host、setup/engine loader、run handle 或 config → rules → start transition 變更必須再執行兩個 startability gates。Game platform schema/scanner/runner 變更至少執行 game-platform、Hub Functions、gamerunner、Cloudflare deploy 與 SEO/noindex 驗證。

## 11. 完成定義

架構優化完成需同時滿足：

- Git `HEAD/index/refs` coherent、沒有 sync-conflict artifacts；可從 `f67977d` recovery anchor 清楚重建並審查每個後續垂直切片。
- 10 個指定模組都由 module-owned engine 實作統一 `TrainingRunHandle`；jsPsych plugin/timeline 是可選實作，不是必要條件。
- setup registry 中每個 native module 都在 CI 的真實 Chromium 中從 config 到 enabled Start，再進入有內容的 running mount；沒有 module 可永久停在 loading/starting/空白 iframe，且 fresh 第二局不受第一局狀態污染。
- card/config 階段不下載或初始化任何 heavy engine；規則顯示是唯一 engine preload trigger。使用者明確發起且只驗 hash/寫 cache 的離線包下載是唯一例外。
- 每個 run 都在 fresh iframe realm 內由 module run handle 擁有 engine/renderer/model/media state，以及模組若有採用的 jsPsych state；host 不直接操作任何 framework-specific state，退出後移除 document 並使資源為零存活。
...
- PR 與 main deploy 的 verify matrix 使用相同 repository-health、training-startability-state 與 official-training-runtime-browser 命令，任一失敗都阻止部署。
...
- 對選擇 jsPsych 8 的既有或新模組，custom plugin 的 `trial()` 應以 `finishTrial()` 或 async return 結束，並可在 async loading 完成後呼叫 `on_load`；這是該 framework 的實作依據，不是平台生命週期的必要條件：<https://www.jspsych.org/v8/developers/plugin-development/>
...
本輪新增 gate：`test:media-lifecycle` 驗證共用 camera/microphone preflight 會立即停止權限探測 stream，module 的實際 stream 只在 `startRun()` 後建立並於 dispose 停止（採用 jsPsych 的模組可在 trial start 建立）；`test:runtime-build-manifest`（由 `build:hub` 在 runtime build 後執行）驗證 Vite root static closure 不含 heavy chunk，並檢查 dynamic module asset path 與 manifest 完整性。

本次 recovery 另加入 `test:repository-health`、`test:training-startability-state` 與 `test:official-training-runtime-browser`；前兩者分別鎖定 Git 基線可解析性與 preload success transition，後者補上既有 source/build gates 無法證明的真實使用者啟動路徑。2026-09-01 的 production build/browser 驗收曾攔到 `reading-training` 的空 timeline 假完成，修正跨 bundler story imports 後，所有 registry module 與 fresh 第二局均已通過；這不解除 canonical Git recovery 的剩餘工作。
