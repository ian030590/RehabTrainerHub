...
- `apps/motortrainer`, `apps/visiontrainer`, `apps/braintrainer`: Vite React trainer apps.
...
- `npm run dev:hub`, `npm run dev:motor`, `npm run dev:vision`, `npm run dev:brain`: run one app locally.
- `npm run build`: build all apps through `scripts/build-apps.mjs`.
- `npm run build:cloudflare`: build Cloudflare Pages outputs.
- `npm run build:hub|motor|mouth|vision|brain`: build one app.
...
Run `npm --workspace @rehab-trainer/motortrainer run test:asteroid-shield-fullscreen` after changes to Asteroid Shield, its fullscreen flow, or Pixi sizing. This regression test verifies the setting and rules flow, native fullscreen target, and full-viewport game canvas.
...
If logic, UI, styling, auth behavior, settings behavior, routing helpers, or footer/navbar behavior can be shared, it must not be reimplemented separately in each trainer. Put reusable code in `packages/ui/src` or another shared helper, then pass only app-specific data such as labels, colors, URLs, or module lists. This is critical because MotorTrainer, VisionTrainer, and BrainTrainer are intentionally parallel products: duplicated logic will drift, create inconsistent accessibility behavior, and require the same bug fix three times. Before editing an app-specific file, check whether the change belongs in `TrainerNavbar`, `TrainerAppLayout`, `AuthPanel`, shared settings utilities, shared CSS, or shared storage/auth helpers. App files should compose shared pieces; they should not fork them.

Player-facing training modules are the exception to broad sharing. A single training experience shown to the player must own its runtime logic, game loop, renderer/canvas lifecycle, input handling, jsPsych/Pixi/Three timeline/plugin setup, and module-specific stimulus or in-game UI styling. Do not centralize or reuse long-lived training engine state across modules, and do not make one module depend on another module's runtime helpers or visual rules. Shared code for these modules should be limited to the React webpage shell: routing, auth, settings forms, layout, navigation, results composition, and renderer-agnostic utilities.
...
Use consistent file and folder names for pages or components with the same role across trainers. For example, route pages should follow shared placement such as `pages/settings/SettingsPage.tsx` and `pages/links/LinksPage.tsx` when the role matches in MotorTrainer, VisionTrainer, and BrainTrainer. Do not keep differently named local files for the same concept unless the behavior is genuinely different; if it is different, keep the name explicit about that app-specific role.
