...
眼動訓練完成時，遊戲把最多 36,000 筆 15 欄逐筆樣本送到 `/api/oculomotor-data`。Functions 驗證欄位、大小、登入或訪客 Subject ID、同源與寫入頻率，產生 UTF-8 BOM CSV 存入 R2。CSV 含逐筆眼動資料、紀錄識別、四種基本活動模式、活動階段、螢幕實體寬高與像素尺寸、觀看距離、五點驗證誤差及當次注視門檻等 metadata；R2 物件 metadata 也保存相同訓練參數，不含總分或彙總成績。WebGazer 先進行九點校正與五點驗證；Tobii 須先在 Tobii Experience 校正，再進行五點驗證。五個驗證點須都有有效樣本，未通過時可重試或改為不記錄眼動。相同 metadata 也隨 Hub 分數或獨立 PWA 摘要寫入 D1；所有分數與彙總成績繼續只寫入 D1。Hub 的 D1 分數紀錄與 R2 CSV 使用相同紀錄 ID。登入資料依帳號隔離，只有原帳號持 Bearer session 能列出並下載，進度頁提供入口。訪客資料使用獨立 Subject ID 路徑，沒有公開讀取 API；結果頁仍提供當下下載。
