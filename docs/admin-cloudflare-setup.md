...
| Variable | `ANONYMOUS_RECORDS_ENABLED` | 訪客紀錄的 server-side kill switch；正式環境啟用時設為 `1`，事故回復時改回 `0` |

兩個 required flag 都是明確開關，預設為 `0`。正式環境請先同時設定 site key 與 secret，再把需要的旗標改成 `1`；缺少任一 key 時部署環境同步會拒絕啟用 required 模式。訪客紀錄正式開啟時，`TURNSTILE_RECORDS_REQUIRED` 必須與 `ANONYMOUS_RECORDS_ENABLED` 一起設為 `1`。目前 production 採用這組設定；若發生濫用、資料安全或寫入異常，先把 `ANONYMOUS_RECORDS_ENABLED` 改回 `0`。
