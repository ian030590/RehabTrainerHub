...
5. pnpm 11.24.0 是唯一套件管理器。pCloud 使用 `nodeLinker: hoisted`、`packageImportMethod: copy`、`injectWorkspacePackages: true`、`dedupeInjectedDeps: false`、`symlink: false`；pnpm 產生的 `.package-map.json`／`.modules.yaml` 由 pCloud bootstrap 在下一次 install 前清理，避免隱藏檔覆寫 EPERM。
...
- pCloud 的 frozen install 需使用 `pnpm run install:pcloud`，CI/Linux 則使用一般 `pnpm install --frozen-lockfile`。
