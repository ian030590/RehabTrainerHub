...
Flow: `training-overlay-config` → validated settings message → game → score message → `training-overlay-score`. After accepting a score, Hub unmounts the game, exits fullscreen, and displays the shared results component. Every accepted score is sent through `/api/records` into D1 `training_records`, with `runtime_id = hub`; unsigned records use the browser-generated guest pseudonymous `subject_id`, while signed-in records use a separate account-scoped `subject_id` and include `user_id`. The two identifiers are never reused across auth states. Unsigned records never appear in account progress and cannot be retrieved by `subject_id`. The score remains in the existing JSON payload.
